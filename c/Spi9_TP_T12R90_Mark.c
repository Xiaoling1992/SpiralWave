...
double gamma=0;
double rhos[]={0.90}, taus[]={12}, sigmas[]={0.42,  0.44,  0.46,  0.48,  0.5 ,  0.52,  0.54,  0.56,  0.58, 0.6 ,  0.62,  0.64,  0.66,  0.68,  0.7 ,  0.72,  0.74,  0.76, 0.78,  0.8, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98};
...
FILE *fptPhase, *fptPeriod, *fptAnc, *fptWave, *fptMap, *fptPhi;     //fpt: ThT; fpt2: parameter;
fptPhase=fopen("Spi9_Phase_Tau12Rho90.dat", "w");
fptPeriod=fopen("Spi9_Period_Tau12Rho90.dat", "w");
fptAnc=    fopen("Spi9_Anchor_Tau12Rho90.dat", "w");
fptPhi=    fopen("Spi9_PhiAct_Tau12Rho90.dat", "w");
   

for(irho=0; irho<Len_rhos; irho++){
for(itau=0;itau<Len_taus;itau++){
  int PhaseDiagram[Len_sigmas][NumRep];
  double period[Len_sigmas][NumRep], AncSizeStr[Len_sigmas][NumRep], PhiAct[Len_sigmas][NumRep];
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &PhiAct[0][0]);	
...
for(ic=0;ic<Len_cs;ic++){
	...
	sigma=-999;
     ...
     strcpy(filename, "Spi9"); 
     strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100+EPSILON));strcat(filename,num); 
     strcat(filename,"Phi");   sprintf(num, "%d",(int)(sigmas[isigma]*1000+EPSILON) );strcat(filename,num);
...
for(ip=0;ip<NumRep; ip++){         
 ...
 //Start to solve the time-space partial derivative equation//
  while (t<TimLen) { 
   ...
   //Initiating the system at t=0;
    ...
    //Calculate the EVnST at time t+dt
  ...
  }//end of while loop. This while loop sometimes spends more time to complete for some parameters;
  ...
  #pragma omp parallel private(j, it)
  {
	...
	}  //End of categorizing the wave pattern;
    ...
    int state[N*M], cluster[N*M], numerator=0, denominator=0, numerator2=0,CluSiz;
     ...
     PhiAct[isigma][ip]=SumMat_D(&tauNoise[0], N, M, N, M, (tau+tau_off)/2, 't' )/N/M; 
...
PriMatD(&PhiAct[0][0], Len_sigmas, NumRep, Len_sigmas, NumRep, fptPhi); 
