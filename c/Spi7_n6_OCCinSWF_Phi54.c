//the spiral travels around a couple of off cell clusters. Mark every cluster. 
...
srand((unsigned) time(&tt));  //Initiating random number generator only once. 
	...
	fptMap=fopen("/home/xiaoling/SpiralWave/data/Spi7_n5Rho0t240p540c10_map.dat","r"); //It must be "r"
	fptOCC=fopen("/home/xiaoling/SpiralWave/data/Spi7_n5Rho0t240p540c10_OCCin_2.dat", "w");
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
    ...
    int state[N*M], cluster[N*M], CluSiz;
		 //Find out the convex hull of the spiral head
		  ...
		  double ConHull[2][NumCMP], SetX[2][10]={ {42, 46, 39, 38, 30, 23, 19, 24, 30, 38},{78, 77, 71, 66, 67, 70, 75, 78, 80, 79}};
