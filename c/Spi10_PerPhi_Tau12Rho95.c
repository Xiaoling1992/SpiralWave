...
double gamma=0;
double rhos[]={0.95}, taus[]={12}, phis[]={0.44,  0.46,  0.48,  0.5 ,  0.52,  0.54,  0.56};

int  NumRep=800, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]);
...
int N=200,M=200, Leng=N*M, coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.
...
fptPhase=fopen("Spi10_Phase_Tau12Rho95_1.dat", "w");
fptPeriod=fopen("Spi10_Period_Tau12Rho95_1.dat", "w");
fptAnc=    fopen("Spi10_Anchor_Tau12Rho95_1.dat", "w");
fptPhi=    fopen("Spi10_PhiAct_Tau12Rho95_1.dat", "w");
   

for(irho=0; irho<Len_rhos; irho++){
for(itau=0;itau<Len_taus;itau++){
  int PhaseDiagram[Len_phis][NumRep];
  double period[Len_phis][NumRep], AncSizeStr[Len_phis][NumRep], PhiAct[Len_phis][NumRep];
  IniMat_I(Len_phis, NumRep, -1, &PhaseDiagram[0][0]);
  IniMat_D(Len_phis, NumRep, -1, &period[0][0]);	
  IniMat_D(Len_phis, NumRep, -1, &AncSizeStr[0][0]);	
  IniMat_D(Len_phis, NumRep, -1, &PhiAct[0][0]);	
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
    ...
    //int state[N*M], cluster[N*M], numerator=0, denominator=0, numerator2=0,CluSiz;
     ...
     PhiAct[iphi][ip]=SumMat_D(&tauNoise[0], N, M, N, M, (tau+tau_off)/2, 't' )/N/M;        
