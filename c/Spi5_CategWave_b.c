...
int  Len_taus=1, Len_sigmas=1, Len_rhos=1, Len_anchors=1, NumRep=1;
...
FILE *fptPhase, *fptWave;     //fpt: ThT; fpt2: parameter;
...
fptPhase=fopen("Spi461_b_Phase.dat","w");
...
for(ic=0;ic<1;ic++){
	...
	rewind(fptWave);   
     ...
     printf("whether died %d\n", WheDied(&wave[0][0], N, M, TimStep, v_th));
	...
	if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  ...
	  for (j=0; j<M; j++){ 
		   ...
		   FindPeaks(deltat, TimLen, dt_write, trace, TimStep, v_th, &PeakTim[i*M+j][0], LenPeakTime);
	   } 
	   }
     ...
     printf("tau %lf, anchor %lf, FirSamMean %lf, PeakMean %lf category %d period %lf\n", tau, anchor, FirSamMean, PeakMean, PhaseDiagram[itau][isigma], period[itau][isigma] );
