//Spi6*b5.1: period and convex hull: find out all the cells with the most peaks (MCP) and peaks of MCP-1.  
...
double PerCelMaxPea(double *PerCMPPre, int NumCMP);
...
double gamma=0;
double taus[]={10}, sigmas[]={1};
double anchor, anchors[]={4}; //This is the diameter of the anchor. 
...
fptPhase=fopen("Spi6_b5_Phase_AncTau_sig.dat","w");
fptPeriod=fopen("Spi6_b5_Period_AncTau_sig.dat", "w");
fptAnc=fopen("Spi6_b5_Anchor_AncTau_sig.dat", "w");
...
for(ic=0;ic<1;ic++){
	...
	fptMap=fopen("Spi47_bRho0t44p38c10_map.dat","r");
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
    ...
    fptWave=fopen("Spi47_bRho0t44p38c10_wave.dat","r");
	rewind(fptWave);   
 ...
 //Categorize the wave     
  ...
  if ( LocEnd== -1 )
       PhaseDiagram[itau][isigma]=0;     //died
  else{
	...
	PeakMean/=NumSample;
     ...
     printf("tau %lf,  anchor %lf, sigma %lf, FirSamMean %lf, PeakMean %lf category %d\n", tau, anchor, sigmas[isigma],  FirSamMean, PeakMean, PhaseDiagram[itau][isigma]);
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  int  NoMaxPea=0, MaxPea=0,  peaks_k, count=0, LocSta=(int)(50/dt_write+EPSILON),NumCMP=0, CelMaxPeak[2][N*M];
	  double PerCMP[N*M];
	...
	for (i=0; i<N; i++){  
	  ...
	  printf("2. MaxPeaks %d\n", MaxPea);
	   //find out all cells with the most peaks (CMP) or with peas of CMP-1
	  for(k=0; k<N*M; k++){
			  if ((int)(PeakTim[k][LenPeakTime-1]+ EPSILON2)== MaxPea || (int)(PeakTim[k][LenPeakTime-1]+ EPSILON2)== MaxPea-1){  //I can't compare a float with an int. 
	  ...
	  if (NumCMP<=1)
	    period[itau][isigma]=PerCMP[0];
	  else  
	    period[itau][isigma]=PerCelMaxPea(PerCMP,  NumCMP); //The order of PerCMP is changed by the deviation.
	 
	  printf("mean period %lf number of picked cells %d\n", period[itau][isigma], NumCMP);	  
	 
	  
	  if(NumCMP<10)  //If the cells with most peaks are too few, instead of pick up these cell with less peaks, we will return failure.
	     AncSize[itau][isigma]=0;
	  else{	     
...
double PerCelMaxPea(double *PerCMPPre, int NumCMP){
	int i=0, j;
	double mean=0, PerCMP[NumCMP], deviation[NumCMP];
	
	for(i=0; i<NumCMP; i++)
	    PerCMP[i]=PerCMPPre[i];
	
	mean=0;  //Initiate the accumulative variable every time right before using it.
	for (i=0; i<NumCMP; i++)
		mean+= PerCMP[i];
	mean/= NumCMP;
...
int ConvexHull(double *SetX, int SizeSetX, int length, double *ConHull, int LenConHull){
	...
	for (i=1; i<SizeSetX; i++){
	 ...
	 puts("sorted array");
	 for (i=0; i<SizeSetX; i++)
	   printf("%lf %lf %lf\n", SorPoi[i].xaxis, SorPoi[i].yaxis, SorPoi[i].cos);
