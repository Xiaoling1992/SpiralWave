...
for(ianchor=0; ianchor<Len_anchors; ianchor++){
...
for(ic=0;ic<1;ic++){
  ...
  //Categorize the wave
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  ...
	  IniMat_D(N*M, LenPeakTime, TimLen+1, &PeakTim[0][0]);
	  ...
	  for (k=0; k<N*M; k++){
		  if (PeakTim[k][LenPeakTime-1]> PeakTim[NoMaxPea][LenPeakTime-1])
		    NoMaxPea=k;}	  
	  MaxPea=(int) (PeakTim[NoMaxPea][LenPeakTime-1]+EPSILON);  //Notice the data type.
	  
	  printf("2. MaxPeaks %d\n", MaxPea);
	   //find out 20 cells with the most peaks
	 for (peaks=MaxPea; peaks>=2 && count<NumCMP; peaks--){
		  for(k=0; k<N*M && count< NumCMP; k++){
			  if ((int)(PeakTim[k][LenPeakTime-1]+ EPSILON2)== peaks){  //I can't compare a float with an int. 
...
puts("after printing PhaseDiagram\n");
...
int ConvexHull(double *SetX, int SizeSetX, int length, double *ConHull, int LenConHull){
	...
	for (i=2; i<SizeSetX; i++){
	  ...
	  while (j>=2&& ((ConHull[j-1]-ConHull[j-2])*(ConHull[j+LenConHull]- ConHull[j-1+LenConHull])-(ConHull[j-1+LenConHull]-ConHull[j-2+LenConHull])*(ConHull[j]- ConHull[j-1])<= 0) ){ //turn left or colinear
