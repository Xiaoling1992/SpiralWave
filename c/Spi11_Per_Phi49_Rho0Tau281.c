...
double gamma=0;
double rhos[]={0}, taus[]={281}, phis[]={0.49};

int  NumRep=100, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]);
...
fptPeriod=fopen("Spi11_Period_Phi49_Rho0Tau281.dat", "w");
fptPhi=    fopen("Spi11_PhiAct_Phi49_Rho0Tau281.dat", "w");
  ...
  double PeakTim[NumSample][LenPeakTime]; 
   ...
   if( WheSpiral ){
	  ...
	  if(NumSpi<=10){
          ...
          strcat(filename,"R");   sprintf(num, "%d", NumSpi); strcat(filename,num);
