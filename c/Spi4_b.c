...
int  Len_means=10, Len_variances=10, Len_rhos=4, NumRep=1;
...
fptPhase=fopen("Spi4_b_Phase.dat","w");
	...
	for(j=0;j<N*M;j++){
     ...
     Ini_Matri(NumSample, LenPeakTime, TimLen+1, &PeakTim[0][0]);  //If no peak, the the peak time is TimLen+1;
       for (k=0; k<NumSample; k++){   //Find out NumSample capable sampling cells
		 ...
		 if(NumTry>=N){
			 j=(int)( N/3*cos(k*2*PI/NumSample)+(M-1)/2.0 +0.5);  
		     i=(int)( N/3*sin(k*2*PI/NumSample)+(N-1)/2.0 +0.5);
		     break;}
