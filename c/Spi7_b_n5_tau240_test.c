...
double gamma=0;
double rhos[]={0}, taus[]={44}, sigmas[]={0.36};
...
double period[Len_sigmas][NumRep], AncSizeStr[Len_sigmas][NumRep], AncSizeDyn[Len_sigmas][NumRep];
fptPhase=fopen("Spi7_n5_Phase_t240_test.dat", "w");
fptPeriod=fopen("Spi7_n5_Period_t240_test.dat", "w");
fptAnc=    fopen("Spi7_n5_Anchor_t240_test.dat", "w");
...
for(itau=0;itau<Len_taus;itau++){
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &AncSizeDyn[0][0]);	
...
for(ic=0;ic<Len_cs;ic++){
	...
	fptWave=fopen("Spi61_b2Rho0t44p360c10_wave.dat","r");
	rewind(fptWave);   
  	for(i=0; i<N*TimStep; i++){
			for(j=0; j<M; j++){
				fscanf( fptWave, "%lf", &wave[i][j] );}    }
   fptMap=fopen("Spi61_b2Rho0t44p360c10_map.dat","r");
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
  ...
  if ( LocEnd== -1 )
    ...
    int state[N*M], cluster[N*M], numerator=0, denominator=0, numerator2=0,CluSiz;
	 ...
	 for (i=0; i<N; i++){
      ...
      printf("Struc anchor %lf\n", AncSizeStr[isigma][ip]);
	...
	for (i=0; i<N; i++){  
	  ...
	  printf("cells with most peaks %d\n", NumCMP);
	  for(k=0; k<NumCMP; k++)
	  ...
	  printf("MaxPeaks %d mean period %lf\n", MaxPea, period[isigma][ip]);
	  
	  // printf("mean period %lf, peaks %d\n", period[isigma][ip], NumCMP);	  
	 
	  //Visulzie the convex hull consisting of all cells with the most peaks.
	  if(NumCMP<5)  //If the cells with most peaks are too few, instead of pick up these cell with less peaks, we will return failure.
	     AncSizeDyn[isigma][ip]=0;
	  else{	     
		  int NumPoiIn;  //Accumulative parameter
	 ...
	 for(k=0; k<NumPoiIn; k++)
	    printf("%lf %lf\n", ConHull[0][k], ConHull[1][k]); 
		 ...
		 int  EffClu[3][N*M/10], End_cluster=0, End_EffClu=0, AncClu=0;
		 ...
		 for (i=0; i<N; i++){
		  ...
		  if (End_EffClu>=1){  //consider all possibilities.
		     AncClu=MaxArrXZ_I(&EffClu[2][0], End_EffClu);	  
		     AncSizeDyn[isigma][ip]= (double)EffClu[1][AncClu];  //notice the data type.
	      }
	      
		  for (k=0; k<End_EffClu; k++)
		    printf("starting %d length %d how many in %d\n", EffClu[0][k], EffClu[1][k], EffClu[2][k]);
		    
		  printf("label of cluster %d start %d dynamic anchor size %lf how many in %d\n", AncClu, EffClu[0][AncClu], AncSizeDyn[isigma][ip], EffClu[2][AncClu] );
		  for(k=EffClu[0][AncClu]; k<EffClu[0][AncClu]+EffClu[1][AncClu]; k++)
	         printf("%d %d\n", cluster[k]/M, cluster[k]%M); 
		  
		 }// End of if it has more than or equal to 10 cells with the most peaks.
...
for(i=0; i<Len_sigmas; i++){ 
 for(j=0;j<NumRep;j++)	
	fprintf(fptAnc,"%lf  ", AncSizeDyn[i][j]);
	fprintf(fptAnc, "\n");	
		         }
