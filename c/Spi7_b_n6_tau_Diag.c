//_Diag: keep the cells of 0th, 2th... diagonal on, the ohters off. Phasediagram and anchor size as a function of tau.
...
double gamma=0;
double rhos[]={0}, taus[]={10, 11, 13, 14, 16, 18, 20, 23, 26, 29, 33, 37, 41, 46, 52, 59, 66, 74, 84, 94, 106, 119, 134, 151, 170, 191, 215, 242, 273, 307, 346, 389, 438, 492, 554, 624, 702, 790, 889, 1000}, sigmas[]={0.5};
...
double period[Len_sigmas][NumRep], AncSizeStr[Len_sigmas][NumRep], AncSizeDyn_n5[Len_sigmas][NumRep], AncSizeDyn_n6[Len_sigmas][NumRep];
fptPhase=fopen("Spi7_Diag_Phase_tau.dat", "w");
fptPeriod=fopen("Spi7_Diag_Period_tau.dat", "w");
fptAnc=    fopen("Spi7_Diag_Anchor_tau.dat", "w");
...
for(itau=0;itau<Len_taus;itau++){
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &AncSizeDyn_n5[0][0]);	
  IniMat_D(Len_sigmas, NumRep, -1, &AncSizeDyn_n6[0][0]);
  ...
  if ( LocEnd== -1 )
	...
	for (i=0; i<N; i++){  
	  ...
	  if(NumCMP<5) { //If the cells with most peaks are too few, instead of pick up these cell with less peaks, we will return failure.
	     AncSizeDyn_n5[isigma][ip]=0;
	     AncSizeDyn_n6[isigma][ip]=0;
	 }
	  else{	     
		 ...
		 for (i=0; i<N; i++){
			 for (j=0; j<M; j++){			 
			    ...
			    End_cluster+=CluSiz; 
			    if(CluSiz >=4){  //Don't consider these little cells to improve the efficiency and reduce the necessary storage.
		  ...
		  AncSizeDyn_n5[isigma][ip]=EffClu[1][ MaxArrXZ_I(&EffClu[2][0], End_EffClu) ];
	      AncSizeDyn_n6[isigma][ip]=EffClu[2][ MaxArrXZ_I(&EffClu[2][0], End_EffClu) ];
	      }
...
for(i=0; i<Len_sigmas; i++){ 
 for(j=0;j<NumRep;j++)	
	fprintf(fptAnc,"%lf  ", AncSizeDyn_n5[i][j]);	
 fprintf(fptAnc, "\n");	
		         }
