...
double tau, taus[]={10, 16, 25, 409};  //\mu of log-normal distribution
double sigma, sigmas[]={4, 8, 12, 16, 20, 24, 28};
double cglobal, cs[]={10};
double gamma=0;
double phi,rho, phis[]={0.4};

int  Len_taus=4, Len_sigmas=1, Len_phis=1, NumRep=1;
...
//fpt=fopen("Spi_T_100by100_Phi.45Rho.6.dat","w");
 //fpt1=fopen("Spi_sta_100by100_Phi.45Rho.6.dat","w");
    ...
    fptPhase=fopen("Spi3_b_Phase_Rho0_test2.dat","w");
...
for(ic=0;ic<1;ic++){
    ...
    CluRadLin(tauNoise, N, M, 'b', rho, phi, tau_off, tau, sigma); 
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
    ...
    puts("End of while loop\n");
     //Find out 8 firing-capable cells
     ...
     Ini_Matri(NumSample, LenPeakTime, TimLen+1, &PeakTim[0][0]);  //If no peak, the the peak time is TimLen+1;
       ...
       for (k=0; k<NumSample; k++){
		 NumTry=0;
		 do{
		 theta=k*2*PI/NumSample+ ( (NumTry+1)/2 )*d_theta* ( (NumTry%2==1)-(NumTry%2 ==0) );
		 ...
		 NumTry++;
		   ...
		   printf("k %d, i %d, j %d, theta %lf, tau %lf, first peak time %lf, peak numbers %lf\n", k, i, j, theta, tauNoise[i*M+j], PeakTim[k][0], PeakTim[k][LenPeakTime-1]);
		   } 
