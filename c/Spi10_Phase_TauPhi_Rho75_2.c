//Spi10* _2: tau_on: 5 to 1000(tau_off=5)
...
fptPhase=fopen("Spi10_Phase_TauPhi_Rho75_2.dat", "w");
...
for(ip=0;ip<NumRep; ip++){         
  ...
  do{
    ...
    fraction=SumMat_D(&tauNoise[0], N, M, N, M, (tau+tau_off)/2, 't' )/N/M;        //Visulize the PhiAct only when it is a spiral.	
    } while(fraction < phi-0.01 || fraction>=phi+0.01);    
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  ...
  if ( LocEnd== -1 )  //LocEnd is int, comparison is good.
       ...
       PhaseDiagram[iphi][ip]=8; //spiral
  else
       PhaseDiagram[iphi][ip]=1; //normal	   
