//Input Spi46_bd()t()tf()c()_wave.dat
//Spi4.9: New method to categorize the wave pattern: Consider all the cells and calculate the ratio of firing cells before the wave reaches the boundary.
//Spi4.7: calculate the period
//_3: FirSamMean_th=0.64
//_1: enlarge the radius of sampling cells and have more samples. r=9/19.
...
double tau, tau_off, phi, sigma, rho, rhos[]={0};
double cglobal, cLow, cs[]={10};
double gamma=0;
double taus[]={10, 16, 26, 42}, sigmas[]={1};
double anchor, anchors[]={4, 8, 12, 16, 20, 24, 28, 32, 36, 40}; //This is the diameter of the anchor. 

int  NumRep=1, Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]), Len_rhos=1, Len_anchors=sizeof(anchors)/sizeof(anchors[0]);
int i,j, ia, ic, itau, ianchor, isigma, irho, ip; //i, i is only used for any short, self-closed loop. 
...
int N=100,M=100, Leng=N*M, print=1,coefx=1,coefy=1;   
...
double deltat=0.002,t=0.00,TimLen=200.00,deltax=1, deltay=1, dt_write=2;          //$$ TimLen incresed to 400 min. 
...
FILE *fptPhase, *fptMap, *fptPeriod, *fptWave, *fptAnc;     //fpt: ThT; fpt2: parameter;
...
double wave[N*TimStep][M], aNoise[N*M],tauNoise[N*M],cNoise[N*M];  //Used only for test. Please
int PhaseDiagram[Len_taus][Len_sigmas];
double period[Len_taus][Len_sigmas], AncSize[Len_taus][Len_sigmas];
fptPhase=fopen("Spi6_b_PhaseAncTau.dat","w");
fptPeriod=fopen("Spi6_b_PeriodAncTau.dat", "w");
fptAnc=fopen("Spi6_b_AnchorAncTau.dat", "w");
...
for(ianchor=0; ianchor<4; ianchor++){
  ...
  IniMat_I(Len_taus, Len_sigmas, -1, &PhaseDiagram[0][0]);
  IniMat_D(Len_taus, Len_sigmas, -1.0, &period[0][0]);
  IniMat_D(Len_taus, Len_sigmas, -1.0, &AncSize[0][0]);
...
for(ic=0;ic<1;ic++){
	...
	sigma=-999;
     ...
     strcpy(filename, "Spi46_b"); 
      strcat(filename,"d");   sprintf(num, "%d",(int)(anchor+EPSILON) );strcat(filename,num);     
     strcat(filename,"t");   sprintf( num, "%d",(int)(tau+EPSILON) );    strcat(filename,num);
     strcat(filename,"tf");   sprintf( num, "%d",(int)(tau_off+EPSILON) ); strcat(filename,num);     
	...
	fptWave=fopen(filename, "r");  //focus : it must be r.
	fptMap=fopen(filename2,"r");   //foucus :it must be r.
	rewind(fptWave);   
  	for(i=0; i<N*TimStep; i++){
			for(j=0; j<M; j++){
				fscanf( fptWave, "%lf", &wave[i][j] );}    }
	
    rewind(fptMap);
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
				fscanf( fptMap, "%lf", &aNoise[i*M+j] );}    } 
	for(i=0; i<N; i++){
			for(j=0; j<M; j++){
				fscanf( fptMap, "%lf", &tauNoise[i*M+j] );}    }
	for(i=0; i<N; i++){
			for(j=0; j<M; j++){
				fscanf( fptMap, "%lf", &cNoise[i*M+j] );}    }  
  ...
  //Categorize the wave
     int k, it, NumSample=(int)(2.8*N), LenPeakTime=30;
     double theta, radius=9.0/19.0*N, trace[TimStep], PeakTim[N*M][LenPeakTime];     
     double FirSamMean=0, FirSamMean_th=0.64, PeakMean=0, PeakMean_th=1.4;
    // puts("breaking point 1");        
     // puts("breaking point 2");
     IniMat_D(NumSample, LenPeakTime, TimLen+1, &PeakTim[0][0]);  //If no peak, the the peak time is TimLen+1;
       for (k=0; k<NumSample; k++){   //Find out NumSample capable sampling cells		     
		 ...
		 j=(int)( radius*cos(theta)+(M-1)/2.0 +0.5);  
		 i=(int)( radius*sin(theta)+(N-1)/2.0 +0.5);
				  
		   for (it=0; it< TimStep; it++)
		     trace[it]=wave[i+it*N][j];
		   //FindPeaks(double deltat, double TimLen, double dt_write, double *trace, int LocSta, int LocEnd, const int TimStep, double v_th, double *PeakTim, int LenPeakTime);  
		   FindPeaks(deltat, TimLen, dt_write, trace, 0, TimStep, TimStep, v_th, &PeakTim[k][0], LenPeakTime);
		   } 
	 ...
	 //PeakTimMean/=NumSample;
	  ...
	  printf("FirSamMean %lf, mean peaks %lf\n", FirSamMean, PeakMean);
     ...
     //Calculate the period of the spiral wave: the period of the cell with the most peaks.  
     printf("1. categotory %d\n", PhaseDiagram[itau][isigma]);
    
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  int  NoMaxPea=0, MaxPea=0, peaks, peaks_k, RanNum_k, count=0, LocSta=(int)(50/dt_write+EPSILON),NumCMP=20, CelMaxPeak[2][NumCMP];
	  double PerCMP[NumCMP];
	  for (i=0; i<N; i++){  
	  for (j=0; j<M; j++){ 
			for (it=0; it< TimStep; it++)
		     trace[it]=wave[i+it*N][j];
	   	     FindPeaks(deltat, TimLen, dt_write, trace, LocSta ,TimStep,TimStep, v_th, &PeakTim[i*M+j][0], LenPeakTime);
	   } 
	   }
	  ...
	  for (k=0; k<N*M; k++){
		  if (PeakTim[k][LenPeakTime-1]> PeakTim[NoMaxPea][LenPeakTime-1])
		    NoMaxPea=k;}	  
	  MaxPea=PeakTim[NoMaxPea][LenPeakTime-1];
	  
	  printf("2. MaxPeaks %d\n", MaxPea);
	   //find out 20 cells with the most peaks
	  for (peaks=MaxPea; peaks>=1 && count<NumCMP; peaks--){
		  for(k=0; k<N*M*10 && count< NumCMP; k++){
			  RanNum_k=rand()%(N*M);  //It is fine if the same is chosen a coupel of times.
			  if (PeakTim[RanNum_k][LenPeakTime-1]== peaks){
				  CelMaxPeak[0][count]= RanNum_k/M;  //i=k/M, x axis
				  CelMaxPeak[1][count]= RanNum_k%M;  //j=k%M, y axis
				  peaks_k=(int)(PeakTim[RanNum_k][LenPeakTime-1]+ EPSILON2);
				  PerCMP[count]= (PeakTim[k][ peaks_k-1] -PeakTim[k][0])/(peaks_k-1);
				  count++;   //Don't forget initiate: for these accumulative parameter. 
			  }
		  }
	  }
	  ...
	  if (count <NumCMP){
	    puts("there is no enough cells contributing the spiral wave");	  
	    break;}
	    
	  for(k=0; k<NumCMP; k++)
	  printf("Before calculating period i %d, j %d, period %lf\n", CelMaxPeak[0][k], CelMaxPeak[1][k], PerCMP[k]); 
	  
	    
	  period[itau][isigma]=PerCelMaxPea(PerCMP,  NumCMP); //The order of PerCMP is changed by the deviation.
	 
	  printf("mean period %lf\n", period[itau][isigma]);
	  for(k=0; k<NumCMP; k++)
	  printf("i %d, j %d, period %lf\n", CelMaxPeak[0][k], CelMaxPeak[1][k], PerCMP[k]);
	  
	  
	  int NumPoiIn, ABLR[4], ConHulSize=0;  //Accumulative parameter
	  double CelMaxPeak_D[2][NumCMP], ConHull[2][NumCMP];
	  for(k=0; k<NumCMP; k++){
		  CelMaxPeak_D[0][k]=(double)CelMaxPeak[0][k];
		  CelMaxPeak_D[1][k]=(double)CelMaxPeak[1][k];}
	   
	  NumPoiIn=ConvexHull(&CelMaxPeak_D[0][0], NumCMP, NumCMP, &ConHull[0][0], NumCMP); //ConvexHull(double *SetX, int SizeSetX, int length, double *ConHull, int LenConHull)
	  
	  for(k=0; k<NumPoiIn; k++)
	  printf("i %lf, j %lf\n", ConHull[0][k], ConHull[1][k]);
	  
	  //Calculate how many cells is in the convex hull
	  ABLR[0]= CelMaxPeak[0][MinArrXZ_I(&CelMaxPeak[0][0], NumCMP)];
	  ABLR[1]= CelMaxPeak[0][MaxArrXZ_I(&CelMaxPeak[0][0], NumCMP)];
	  ABLR[2]= CelMaxPeak[1][MinArrXZ_I(&CelMaxPeak[1][0], NumCMP)];
	  ABLR[3]= CelMaxPeak[1][MaxArrXZ_I(&CelMaxPeak[1][0], NumCMP)];
	  
	  for(k=0; k<4; k++)
	  printf("ABRL %d", ABLR[k] );	  
	  puts("\n");
	  
	  for(i= ABLR[0]; i<= ABLR[1]; i++){
		  for(j= ABLR[2]; j<= ABLR[3]; j++){  //Area of the anchor: All the off cells inside the anchor.
			ConHulSize+= (tauNoise[i*M+j]==tau_off)*WheInConHull(&ConHull[0][0], NumPoiIn, NumCMP, (double)i, (double)j);//int WheInConHull(double *ConHull, int NumPoints, int LenConHull, double x, double y);  }
		  }
	  }
	  
	  AncSize[itau][isigma]=ConHulSize;	
	  printf("anchor size %lf\n", AncSize[itau][isigma]);  
	  

  } 
	    	   
  
     printf("tau %lf, sigma %lf, anchor %lf, FirSamMean %lf, PeakMean %lf category %d period %lf\n", tau, sigma, anchor, FirSamMean, PeakMean, PhaseDiagram[itau][isigma], period[itau][isigma] );
...
int ConvexHull(double *SetX, int SizeSetX, int length, double *ConHull, int LenConHull){
	...
	printf("A0: %lf %lf\n", SorPoi[A0].xaxis, SorPoi[A0].yaxis);
	...
	for (i=1; i<SizeSetX; i++){
	 ...
	 for (i=0; i<SizeSetX; i++)
	   printf("%lf %lf  ", SorPoi[i].xaxis, SorPoi[i].yaxis);
	 puts("\n");
	...
	for (i=2; i<SizeSetX; i++){
	  ...
	  while ( (ConHull[j-1]-ConHull[j-2])*(ConHull[j+LenConHull]- ConHull[j-1+LenConHull])-(ConHull[j-1+LenConHull]-ConHull[j-2+LenConHull])*(ConHull[j]- ConHull[j-1])<= 0){ //turn left or colinear
