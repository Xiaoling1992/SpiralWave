...
double gamma=0;
double rhos[]={0}, taus[]={240}, sigmas[]={0.430, 0.434, 0.438, 0.443, 0.449, 0.456, 0.464, 0.474, 0.485, 0.497, 0.512, 0.529, 0.549, 0.573, 0.600};
...
int Ns[]={100, 200, 400, 800}, Len_Ns=sizeof(Ns)/sizeof(Ns[0]), iN;
...
int N=Ns[iN],M=Ns[iN], Leng=N*M, WinHei=35, print=1,coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.
...
double AncSizeStr_n1[Len_sigmas][NumRep], AncSizeStr_n2[Len_sigmas][NumRep];
...
for(itau=0;itau<Len_taus;itau++){
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &AncSizeStr_n1[0][0]);	
  IniMat_D(Len_sigmas, NumRep, -1, &AncSizeStr_n2[0][0]);	
...
for(ip=0;ip<NumRep; ip++){         
    ...
    int state[N*M], cluster[N*M], numerator=0, denominator=0, numerator2=0,CluSiz;
	 ...
	 for (i=0; i<N; i++){
	  ...
	  if(denominator >=1){
      AncSizeStr_n1[isigma][ip]=(double) numerator/ (double)denominator;  
      AncSizeStr_n2[isigma][ip]=(double) numerator2/ (double)numerator;   //S=n1^2+ n2^2 n3^3.../n1+n2+n3...
     }
...
PriMatD(&AncSizeStr_n1[0][0], Len_sigmas, NumRep, Len_sigmas, NumRep, fptAnc);		         
PriMatD(&AncSizeStr_n2[0][0], Len_sigmas, NumRep, Len_sigmas, NumRep, fptAnc);
