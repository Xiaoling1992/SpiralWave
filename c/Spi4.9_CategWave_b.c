...
int FindPeaks(double deltat, double TimLen, double dt_write, double *trace, int LocSta, int LocEnd, int TimStep, double v_th, double *PeakTim, int LenPeakTime);
...
int  Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]), Len_rhos=1, Len_anchors=1, NumRep=1;
...
double period[Len_taus][Len_sigmas];
fptPhase=fopen("Spi49_b_Phase.dat","w");
fptPeriod=fopen("Spi49_b_Period.dat", "w");
...
for(ianchor=0; ianchor<4; ianchor++){
  ...
  Ini_Matri(Len_taus, Len_sigmas, -1.0, &period[0][0]);
...
for(ic=0;ic<1;ic++){
  ...
  if ( LocEnd== -1 )
	  ...
	  for (j=0; j<M; j++){ 
			for (it=0; it< TimStep; it++)
		     trace[it]=wave[i+it*N][j];
	   	     FindPeaks(deltat, TimLen, dt_write, trace, LocSta ,LocEnd,TimStep, v_th, &PeakTim[i*M+j][0], LenPeakTime); //consider only the trace before the LocEnd
	   } 
	   }
	 ...
	 if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  int NoMaxPea=0, MaxPea=0;
	  LocSta=(int)( 100/dt_write+ EPSILON2);
	  LocEnd=TimStep;
	  for (i=0; i<N; i++){  
	  for (j=0; j<M; j++){ 
			for (it=0; it< TimStep; it++)
		    ...
		    FindPeaks(deltat, TimLen, dt_write, trace, LocSta ,LocEnd, TimStep, v_th, &PeakTim[i*M+j][0], LenPeakTime);
	   } 
	   }
	  ...
	  for (k=0; k<N*M; k++){
	      ...
	      period[itau][isigma]=(PeakTim[NoMaxPea][MaxPea-1]- PeakTim[NoMaxPea][0])/(MaxPea-1);
	   	  }
	}
