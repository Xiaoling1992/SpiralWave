...
double aglobal, aLow, as[]={0.02};  ////phi: percentage of responsive cells; rho, clustering rate, aLow, the parameter of unresponsive cell.                                                 
double tau, tau_off, phi, sigma, rho;
double cglobal, cLow, cs[]={10};
double gamma=0;
double rhos[]={0}, taus[]={300}, sigmas[]={1.0,1.2,1.4,1.6,1.9,2.2,2.6,3.0,3.6,4.2,4.9,5.7,6.7,7.9,9.2,10, 10.8,12.7,14.9,17.4,20.4,24.0,28.1,32.9,38.6,45.2,53.0,62.1,72.8,85.3,100.0};

int NumRep=100, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]);
...
fptPhase=fopen("Spi6_b_Epsilon2_Phase.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	sigma=-999;
     ...
     strcat(filename,"tf");   sprintf( num, "%d",(int)(tau_off*10+EPSILON) );    strcat(filename,num);
     strcat(filename,"p");   sprintf(num, "%d",(int)(phi*1000+EPSILON) ); strcat(filename,num);
     strcat(filename,"a");   sprintf(num, "%d",(int)(aglobal*1000)); strcat(filename,num);
     strcat(filename,"c");   sprintf(num, "%d",(int)(cglobal*10+EPSILON2)); strcat(filename,num);
