...
double gamma=0;
double rhos[]={0.50}, taus[]={12}, sigmas[]={0.42,  0.44,  0.46,  0.48,  0.5 ,  0.52,  0.54,  0.56,  0.58, 0.6 ,  0.62,  0.64,  0.66,  0.68,  0.7 ,  0.72,  0.74,  0.76, 0.78,  0.8};

int  NumRep=800, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]);
...
fptPhase=fopen("Spi8_Phase_t12Rho50_test.dat", "w");
fptPeriod=fopen("Spi8_Period_t12Rho50_test.dat", "w");
fptAnc=    fopen("Spi8_Anchor_t12Rho50_test.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	sigma=-999;
     ...
     strcat(filename,"_wavetest.dat");	
    strcat(filename2,"_maptest.dat");
