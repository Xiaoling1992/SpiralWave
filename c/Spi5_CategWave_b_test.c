...
for(itau=15;itau<18;itau++){	
for(isigma=4;isigma<12;isigma++){ 
...
for(ic=0;ic<1;ic++){
	...
	rewind(fptWave);   
     ...
     int k, it, NumSample=(int)(2.2*N), LenPeakTime=10, itReach;
     double theta, dr=1, rvar,radius=9.0/20.0*N, RMin=2.0/5*N, RMax=1.0/2*(N-1), trace[TimStep], PeakTim[NumSample][LenPeakTime+1];   //0:LenPeakTime-2, store peak time; LenPeakTime-1, store the #peaks; LenPeakTime, store the whether a firing cell is found in this direction (theta) at time tReach.
     double FirSamMean=0, FirSamMean_th=0.6, PeakMean=0, PeakMean_th=1.4, tReach, FirWFMean=0, FirWFMean_th=0.6;
	 ...
	 //PeakTimMean/=NumSample;
      ...
      if ( WheDied(&wave[0][0], N, M, TimStep, v_th) )
       PhaseDiagram[itau][isigma]=0;
		...
		tReach=PeakTim[0][0];
		for(i=0; i<NumSample; i++){
			if (tReach> PeakTim[k][0])
			 tReach= PeakTim[k][0];}
		printf("tReach %lf, time of Peak 0 S0 %lf S1 %lf, S2 %lf, S3, %lf \n", tReach, PeakTim[0][0],PeakTim[1][0],PeakTim[2][0],PeakTim[3][0]);
	...
	if(FirWFMean/NumSample< FirWFMean_th){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[itau][isigma]=2;//Spiral-brek up
		 else
		   PhaseDiagram[itau][isigma]=3;}		   
