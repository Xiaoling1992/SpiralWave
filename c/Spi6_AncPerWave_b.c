...
for(ip=0;ip<NumRep; ip++){         
    ...
    fptWave=fopen("Spi47_bRho0t44p38c10_wave.dat","w");
	rewind(fptWave);   
    ...
    if(PhaseDiagram[isigma][ip]==2 || PhaseDiagram[isigma][ip]==8){
	  int  NoMaxPea, MaxPea=0, peaks, peaks_k, count, LocSta=(int)(100/dt_write+EPSILON),NumCMP=20, CelMaxPeak[2][NumCMP];
