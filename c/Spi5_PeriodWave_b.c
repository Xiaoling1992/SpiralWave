//Spi5: validate the algorithm used in Spi5 to calculate the period.
...
FILE *fptPhase, *fptPeriod, *fptWave;     //fpt: ThT; fpt2: parameter;
...
int PhaseDiagram[Len_taus][Len_anchors];
double period[Len_taus][Len_anchors];
fptPhase=fopen("Spi5_PhaSpi46_val.dat","w");
fptPeriod=fopen("Spi5_PerSpi46_val.dat", "w");
   
  

for(ip=0;ip<1; ip++){
for(irho=0; irho<Len_rhos; irho++){
  Ini_Matri_I(Len_taus, Len_anchors, -1, &PhaseDiagram[0][0]);
  Ini_Matri(Len_taus, Len_anchors, -1.0, &period[0][0]);
    ...
    // puts("breaking point 1");        
	 ...
	 //PeakTimMean/=NumSample;
      ...
      if ( WheDied(&wave[0][0], N, M, TimStep, v_th) )
       PhaseDiagram[itau][ianchor]=0;
     else if( FirSamMean< FirSamMean_th){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[itau][ianchor]=2;//Spiral-brek up
		 else
		   PhaseDiagram[itau][ianchor]=3;}
     else if( PeakMean>= PeakMean_th-EPSILON2 ){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[itau][ianchor]=8;  //spiral_secondary
		 else
		   PhaseDiagram[itau][ianchor]=9;}  //secondary
	 else
	    PhaseDiagram[itau][ianchor]=5;     //normal
       
     //Calculate the period of the spiral wave: the period of the cell with the most peaks.  
    
    if(PhaseDiagram[itau][ianchor]==2 || PhaseDiagram[itau][ianchor]==8){

	  int NoMaxPea=0, MaxPea=0, LocSta=(int)(100/dt_write+EPSILON);
	  for (i=0; i<N; i++){  
	  for (j=0; j<M; j++){ 
			for (it=0; it< TimStep; it++)
		     trace[it]=wave[i+it*N][j];
	  ...
	  for (k=0; k<N*M; k++){
	      ...
	      period[itau][ianchor]=(PeakTim[NoMaxPea][MaxPea-1]- PeakTim[NoMaxPea][0])/(MaxPea-1);
     ...
     printf("tau %lf, anchor %lf, FirSamMean %lf, PeakMean %lf category %d period %lf\n", tau, anchor, FirSamMean, PeakMean, PhaseDiagram[itau][ianchor], period[itau][ianchor] );
