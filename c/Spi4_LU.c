...
fptPhase=fopen("Spi4_LU_Phase.dat","w");
...
for(ic=0;ic<1;ic++){
	...
	ParaLU(means[imean], variances[ivariance], &tau, &sigma);
    ...
    CluRadLin(tauNoise, N, M, 'U', rho, -999, -999, tau, sigma); //-999 means that input is not necessary in the function.
	...
	for(j=0;j<N*M;j++){
     ...
     Ini_Matri(NumSample, LenPeakTime, TimLen+1, &PeakTim[0][0]);  //If no peak, the the peak time is TimLen+1;
       for (k=0; k<NumSample; k++){   //Find out NumSample capable sampling cells
		 ...
		 NumTry++;
		  } while( tauNoise[i*M+j]< exp(tau) );   //exp(tau for lognormal and loguniform distribution)
