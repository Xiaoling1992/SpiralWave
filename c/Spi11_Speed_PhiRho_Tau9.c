...
double gamma=0;
double rhos[]={0.74, 0.76, 0.78, 0.80, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.0, 0}, taus[]={9}, phis[]={0.42,  0.44,  0.46,  0.48,  0.5 ,  0.52,  0.54,  0.56,  0.58, 0.6 ,  0.62,  0.64,  0.66,  0.68,  0.7 ,  0.72,  0.74,  0.76, 0.78,  0.8, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.0};

int  NumRep=10, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]);
...
double deltat=0.01,t=0.00,TimLen=50.00,deltax=1, deltay=1, dt_write=2;          //$$ TimLen incresed to 400 min. 
...
fptPeriod=fopen("Spi11_Speed_PhiRho_Tau9.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	phi=phis[iphi];    
     ...
     strcat(filename,"_speed_wave.dat");	
    strcat(filename2,"_speed_map.dat");
...
for(ip=0;ip<NumRep; ip++){         
    ...
    int WheSpiral;     
	t=0;
    double time_55= 0, time_95= 0, dis= 40;
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) {            
	...
	for(j=0;j<N*M;j++){
      ...
      dxdt_Squ_o4(&EVnST1[0][0],&k1[0][0],aNoise,tauNoise,cNoise,gamma,coefx,coefy,N,M,Leng,deltax,deltay);  
       ...
       dxdt_Squ_o4(&EVnST1[0][0],&k3[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay); 
	    ...
	    //if(WheMultiple(t, dt_write, deltat/10)|| equal(t,2*deltat,deltat/10) ){ //If it is not a spiral: terminate.
			//for (i=0; i<N; i++){
			  //for (j=0;j<M;j++)
        ...
        //Check whether (50, 55), (50, 95) are firing: it may be off cell
        if (time_55==0 && EVnST[0][50*M+ 55]>= v_th   )
            time_55= t;
        else if (time_95==0 && EVnST[0][50*M+95]>= v_th )
            time_95= t;
    ...
    if (time_95> time_55)
        period[iphi][ip]= dis/(time_95- time_55);
