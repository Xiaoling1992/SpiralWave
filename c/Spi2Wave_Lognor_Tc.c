...
double aglobal,as[]={0.1};  //WT, trkA, sinR, ktrA

double tau, taus[]={100};  //\mu of log-normal distribution
double sigma, sigmas[]={10};
double cglobal, cs[]={10};
double gamma=0, phi_real;
double phi,rho, phis[]={1};

int  Len_as=1, Len_taus=1, Len_sigmas=1, Len_phis=1, NumRep=1;
...
for(ic=0;ic<1;ic++){
    ...
    strcpy(filename, "test_Spi2b"); 
     ...
     strcat(filename,"tL");   sprintf(num, "%d",(int)(tau_off+EPSILON) );strcat(filename,num);
      ...
      //strcat(filename,"s");  sprintf( num, "%d",(int)(sigma*100) );strcat(filename,num);
      
      strcat(filename,"c");   sprintf(num, "%d",(int)(cglobal*100));strcat(filename,num);
	...
	CluRadLin(aNoise,   N, M, 'N', 0, 0.4, aLow, aglobal, sigma);      
    CluRadLin(tauNoise, N, M, 'b', 0, 0.4, tau_off, tau, 0); 
    CluRadLin(cNoise,   N, M, 'U', 0, 0.4 , cLow, cglobal, sigma);  
...
int cluster[height*width], state[height*width], NClu, NClu0, NEffClu, NEffClu0, NMother, daughter,mother, StoLoc, ABLR,m;
 ...
 if(mode=='s'){
	...
	//Initiation: the first cell in the middle (height/2, width/2)
	mother=height/2*width+width/2;
	if (mode=='b')
