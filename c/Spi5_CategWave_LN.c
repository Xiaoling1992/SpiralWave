//Spi5: use the wavefront at time t* as the dependence to decide whether it is a break up or not. (To tell breaking spiral from secondary spiral.)
...
double gamma=0;
double taus[]={2.00, 2.17, 2.34, 2.52, 2.69, 2.86, 3.03, 3.21, 3.38, 3.55, 3.72, 3.90, 4.07, 4.24, 4.41, 4.59, 
	4.76, 4.93, 5.10, 5.28, 5.45, 5.62, 5.79, 5.97, 6.14, 6.31, 6.48, 6.66, 6.83, 7.00}, sigmas[]={1.00, 1.10, 
    1.21, 1.31, 1.41, 1.52, 1.62, 1.72, 1.83, 1.93, 2.03, 2.14, 2.24, 2.34, 2.45, 2.55, 2.66, 2.76, 2.86, 2.97, 
    3.07, 3.17, 3.28, 3.38, 3.48, 3.59, 3.69, 3.79, 3.90, 4.00};

int  Len_taus=30, Len_sigmas=30, Len_rhos=4, NumRep=1;
...
fptPhase=fopen("Spi5_LN_Phase.dat","w");
...
for(ic=0;ic<1;ic++){
	...
	rewind(fptWave);   
     ...
     int k, it, NumSample=(int)(2.8*N), LenPeakTime=10, itReach;
     double theta, dr=0.1, rvar,radius=9.0/19.0*N, RMin=radius-1, RMax=radius+1, trace[TimStep], PeakTim[NumSample][LenPeakTime+1];   //0:LenPeakTime-2, store peak time; LenPeakTime-1, store the #peaks; LenPeakTime, store the whether a firing cell is found in this direction (theta) at time tReach.
     double FirSamMean=0, FirSamMean_th=0.6, PeakMean=0, PeakMean_th=1.4, tReach, FirWFMean=0, FirWFMean_th=0.60;
         
     //puts("breaking point 2");
     Ini_Matri(NumSample, LenPeakTime+1, TimLen+1, &PeakTim[0][0]);  //If no peak, the the peak time is TimLen+1;
	 ...
	 //PeakTimMean/=NumSample;
	  ...
	  for(k=0; k<NumSample; k++){
	   FirSamMean+= ( PeakTim[k][LenPeakTime-1]>= 1-EPSILON2 );   //Compare two double numbers, reduce the latter one by EPSILON2 to avoid the false negative.
	   PeakMean+=PeakTim[k][LenPeakTime-1];	 
	   }  
      ...
      if ( WheDied(&wave[0][0], N, M, TimStep, v_th) )
       PhaseDiagram[itau][isigma]=0;
		...
		tReach=50;  //Decided by experience
		for(k=0; k<NumSample; k++){
			if (PeakTim[k][0]< tReach && PeakTim[k][0]>=5)
			 tReach= PeakTim[k][0];
			 }
		printf("tReach %lf\n", tReach);
		itReach=(int)(tReach/dt_write+EPSILON2);
			 
		for(k=0; k<NumSample; k++){
			 theta=k*2*PI/NumSample;
			 
			for(rvar=RMax; rvar>=RMin; rvar-=dr ){
				 j=(int)( rvar*cos(theta)+(M-1)/2.0 +0.5);  
		         i=(int)( rvar*sin(theta)+(N-1)/2.0 +0.5);
		         if(wave[i+itReach*N][j] >= v_th){
					 PeakTim[k][LenPeakTime]=1;
					 FirWFMean+=1;
					 break;}
					 }
		}	
	...
	if(FirWFMean< FirWFMean_th){
     ...
     printf("tau %lf, phi %lf, FirSamMean %lf, FirWFMean %lf, PeakMean %lf category %d\n", tau, phi, FirSamMean, FirWFMean, PeakMean, PhaseDiagram[itau][isigma]);
     if (isigma==Len_sigmas)
     puts("\n");
