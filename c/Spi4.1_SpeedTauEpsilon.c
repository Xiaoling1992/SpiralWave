...
//Spi4. Use mean and std as the inputs indtead of parameters of the distribution. Fix a bug in deciding the wave pattern: 
...
int MeaSpeed(double *wave, int height, int width, int TimStep, double threshold, double *distance, double deltay, double *time, double dt_write);
...
double cglobal, cLow, cs[]={1, 2, 3, 5, 8, 13, 22, 36, 60, 100};
double gamma=0;
double means[]={10, 17, 28, 46, 77, 129, 215, 359, 599, 1000},
   stds[]={2, 4, 6, 11, 20, 34, 61, 108, 190, 336, 595, 1051, 1857, 3282, 5800, 10250, 18116, 32016, 56583, 100000};

int  Len_means=10, Len_stds=20, Len_rhos=4, Len_cs=10, NumRep=1;
int i,j, ia, ic, imean, istd, irho, ip; //i, i is only used for any short, self-closed loop. 
...
int N=100,M=3, Leng=N*M, WinHei=35, print=1,coefx=1,coefy=1;   
...
double deltat=0.002,t=0.00,TimLen=200.00,deltax=1, deltay=1, dt_write=0.5;          //$$ TimLen incresed to 400 min. 
...
double PhaseDiagram[Len_means][Len_cs];
fptPhase=fopen("Spi41_SpeedTauC.dat","w");
...
for(irho=0; irho<1; irho++){
  ...
  Ini_Matri(Len_means, Len_cs, -999, &PhaseDiagram[0][0]);
for(imean=0;imean<Len_means;imean++){	//loop for tau
for(istd=0;istd<1;istd++){ 
	   
for(ia=0;ia<1;ia++){
for(ic=0;ic<Len_cs;ic++){  //Loop for Epsilon (c)
    ...
    tau=means[imean]; 
     ...
     strcpy(filename, "Spi41_homo"); 
     strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100+EPSILON));strcat(filename,num); 
     strcat(filename,"v");   sprintf(num, "%d",(int)(aglobal+EPSILON) );strcat(filename,num);
     strcat(filename,"t");   sprintf( num, "%d",(int)(means[imean]+EPSILON) );    strcat(filename,num);
    ...
    CluRadLin(tauNoise, N, M, 's', rho, phi, tau_off, tau, sigma); //-999 means that input is not necessary in the function.
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
   ...
   double time_dis, distance;
   
   if(MeaSpeed(&wave[0][0], N, M, TimStep, v_th, &distance, deltay, &time_dis, dt_write))
    PhaseDiagram[imean][ic]=distance/time_dis;
   else
    PhaseDiagram[imean][ic]=0;
     ...
     //double FirSamMean=0, FirSamMean_th=0.7, PeakMean=0, PeakMean_th=1.4;
	 ...
	 //PeakTimMean/=NumSample;
      ...
      //if ( WheDied(&wave[0][0], N, M, TimStep, v_th) )
       //PhaseDiagram[imean][istd]=0;
     //else if( FirSamMean< FirSamMean_th){
		 //if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   //PhaseDiagram[imean][istd]=2;//Spiral-brek up
		 //else
		   //PhaseDiagram[imean][istd]=3;}
     //else if( PeakMean>= PeakMean_th-EPSILON2 ){
		 //if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   //PhaseDiagram[imean][istd]=8;  //spiral_secondary
		 //else
		   //PhaseDiagram[imean][istd]=9;}  //secondary
	 //else
	    //PhaseDiagram[imean][istd]=5;     //normal
 ...
 if(ip==NumRep-1){
		...
		printf("mean=%lf, std=%lf, tau=%lf tau_off %lf sigma=%lf rho=%lf  c=%lf, cLow=%lf\n",means[imean], stds[istd], tau, tau_off, sigma, rho,  cglobal, cLow);
		for (i=0;i<N*M;i++){ 	
				if(i%M==M-1)
		        fprintf(fptMap,"%lf\n",aNoise[i]);	
		        else
...
//for(itau=0;itau<Len_taus;itau++){ 	
 ...
 for(i=0; i<Len_means; i++){ 
 for(j=0;j<Len_cs;j++)	
	fprintf(fptPhase,"%.4f  ",PhaseDiagram[i][j]);
	fprintf(fptPhase, "\n");	
		         }
