//Spi4. Use mean and variance as the inputs indtead of parameters of the distribution. 
...
double tau, tau_off, phi, rho, rhos[]={0, 0.25, 0.5, 0.75};
double cglobal, cLow, cs[]={10};
double gamma=0;
double means[]={6, 12, 24, 48, 95,189,378,754, 1504, 3000}, variances[]={2.00000000e+00, 9.00000000e+00, 3.70000000e+01, 1.59000000e+02
, 6.82000000e+02, 2.93200000e+03, 1.25990000e+04, 5.41450000e+04
, 2.32692000e+05, 1.00000000e+06};

int  Len_means=2, Len_variances=2, Len_rhos=2, NumRep=1;
int i,j, ia, ic, imean, ivariance, irho, ip; //i, i is only used for any short, self-closed loop. 
...
double deltat=0.01,t=0.00,TimLen=100.00,deltax=1, deltay=1, dt_write=2;          //$$ TimLen incresed to 400 min. 
...
int PhaseDiagram[Len_means][Len_variances];
fptPhase=fopen("test_Spi4_b_Phase.dat","w");
...
for(irho=0; irho<Len_rhos; irho++){
  ...
  Ini_Matri_I(Len_means, Len_variances, -1, &PhaseDiagram[0][0]);
for(imean=0; imean<Len_means; imean++){	
for(ivariance=0; ivariance<Len_variances; ivariance++){ 
...
for(ic=0;ic<1;ic++){
	...
	rho=rhos[irho]; 
	tau_off=5;	
	phi=pow( means[imean]- tau_off, 2 )/( variances[ivariance] +pow( means[imean]- tau_off, 2 ) );
    tau=means[imean]+ variances[ivariance]/(means[imean]- tau_off); 
     
     strcpy(filename, "test_Spi4_b"); 
     strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100+EPSILON));strcat(filename,num); 
     strcat(filename,"m");   sprintf( num, "%d",(int)(means[imean]+EPSILON) );    strcat(filename,num);
     strcat(filename,"v");   sprintf(num, "%d",(int)(variances[ivariance]+EPSILON) );strcat(filename,num);
	...
	CluRadLin(aNoise,   N, M, 's', rho, phi, aLow, aglobal, -1);      
    CluRadLin(tauNoise, N, M, 'b', rho, phi, tau_off, tau, -1); 
    CluRadLin(cNoise,   N, M, 's', rho, phi, cLow, cglobal, -1);  
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
	...
	for(j=0;j<N*M;j++){
     ...
     double FirSamMean=0, FirSamMean_th=0.8, PeakMean=0, PeakMean_th=1.5;
	 ...
	 //PeakTimMean/=NumSample;
      ...
      if ( WheDied(&wave[0][0], N, M, TimStep, v_th) )
       PhaseDiagram[imean][ivariance]=0;
     else if( FirSamMean< FirSamMean_th){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[imean][ivariance]=2;//Spiral-brek up
		 else
		   PhaseDiagram[imean][ivariance]=3;}
     else if( PeakMean>= PeakMean_th-EPSILON2 ){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[imean][ivariance]=8;  //spiral_secondary
		 else
		   PhaseDiagram[imean][ivariance]=9;}  //secondary
	 else
	    PhaseDiagram[imean][ivariance]=5;     //normal
 ...
 if(ip==NumRep-1){
	...
	printf("phi=%lf tau=%lf tau_off%lf rho=%lf mean=%lf, variance=%lf, c=%lf, cLow=%lf\n",phi, tau, tau_off, rho, means[imean], variances[ivariance], cglobal, cLow);
	for (i=0;i<N*M;i++){ 	
				if(i%M==M-1)
		        fprintf(fptMap,"%lf\n",aNoise[i]);	
		        else
