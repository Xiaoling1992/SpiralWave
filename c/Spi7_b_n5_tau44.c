...
double gamma=0;
double rhos[]={0}, taus[]={44}, sigmas[]={0.28, 0.29, 0.30, 0.31, 0.32, 0.33, 0.34,0.35, 0.36, 0.37, 0.38, 0.39, 0.40, 0.41, 0.42};
...
fptPhase=fopen("Spi7_n5_Phase_t44.dat", "w");
fptPeriod=fopen("Spi7_n5_Period_t44.dat", "w");
fptAnc=    fopen("Spi7_n5_Anchor_t44.dat", "w");
  ...
  if ( LocEnd== -1 )
	...
	for (i=0; i<N; i++){  
	  ...
	  if(NumCMP<5)  //If the cells with most peaks are too few, instead of pick up these cell with less peaks, we will return failure.
		 ...
		 //Find out all the off-cell clusters, store these clusters in the cluster. Use EffClu to record the starting point (row 0), 
		 //the length of cluster (row 1), and how many off-cells are inside the convex hull (row 2) (only if the cluster size is larger than 4). 
		 ...
		 for (i=0; i<N; i++){
		  ...
		  AncSizeDyn[isigma][ip]=EffClu[1][ MaxArrXZ_I(&EffClu[2][0], End_EffClu) ];
