//Get the duratin as a function of tau
//Generate 1 wave for WT, trkA, sinR and ktrA.
...
double aglobal,as[]={0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02};  //WT, trkA, sinR, ktrA
double tau, taus[]={10, 12, 14, 16, 19, 22, 26, 30, 36, 42, 49, 57, 67, 79, 92, 108, 127, 149, 174, 204, 240, 281, 329, 386, 452, 530, 621, 728, 853, 1000};  
double cglobal, cs[]={10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10};
double gamma=0, phi_real;
double phi,rho, phis[]={0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43, 0.43};

int   Len_taus=sizeof(taus)/sizeof(taus[0]),NumRep=1, Len_phis=1, Len_as=1;
...
int N=HEIGHT,M=WIDTH,WinHei=35, print=1,noise=1,coefx,coefy;   
...
fpt=fopen("F17_T_Tau.dat","w");
 fpt1=fopen("F17_T_Tau_sta.dat","w");
...
for(itau=0;itau<Len_taus;itau++){  //Keep only one loop parameter
	phi=phis[itau];
    rho=0;    
    ...
    strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100));strcat(filename,num); 
      strcat(filename,"a"); sprintf(num, "%d",(int)(aglobal*100)); strcat(filename,num); 
