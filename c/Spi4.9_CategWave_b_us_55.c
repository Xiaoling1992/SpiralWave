...
double gamma=0;
double taus[]={10, 12, 14, 16, 19, 22, 26, 30, 36, 42, 49, 57, 67, 79, 92, 108, 127, 149, 174, 204, 
	240, 281, 329, 386, 452, 530, 621, 728, 853, 1000}, sigmas[]={0.03, 0.06, 0.10, 0.13, 0.16, 0.20, 
    0.23, 0.26, 0.30, 0.33, 0.36, 0.40, 0.43, 0.46, 0.50, 0.53, 0.57, 0.60, 0.63, 0.67, 0.70, 0.73, 
    0.77, 0.80, 0.83, 0.87, 0.90, 0.93, 0.97, 1.00};


int  NumRep=1, Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]), Len_rhos=sizeof(rhos)/sizeof(rhos[0]);
...
//fpt=fopen("Spi_T_100by100_Phi.45Rho.6.dat","w");
 //fpt1=fopen("Spi_sta_100by100_Phi.45Rho.6.dat","w");
     ...
     //IniMat_D(Len_as, Len_taus,0.00, &WF_ab[0][0]);
     //IniMat_D(Len_as, Len_taus,0.00, &WL_ab[0][0]);
     //IniMat_D(Len_as, Len_taus,0.00, &PhiFir_ab[0][0]);
     //IniMat_D(Len_as, Len_taus,0.00, &TraEff_ab[0][0]);
     //IniMat_D(Len_as, Len_taus,0.00, &T_ab[0][0]);
...
fptPhase=fopen("Spi4.9_b_Phase_us_55.dat","w");
fptPeriod=fopen("Spi4.9_b_Period_us_55.dat", "w");
fptAnc=fopen("Spi4.9_b_Anchor_us_55.dat", "w");
...
for(ic=0;ic<1;ic++){
    ...
    strcat(filename,"_wave.dat");  strcat(filename2,"_map.dat");
  ...
  int LocSta=0, LocEnd=0,  NumSample=N*M, k, it, LenPeakTime=30;
  double trace[TimStep], PeakTim[NumSample][LenPeakTime];     
  double FirSamMean=0, FirSamMean_th=0.55, PeakMean=0, PeakMean_th=1.4;
  ...
  if ( LocEnd== -1 )
      ...
      //Visulize the FirSamMean before the LocEnd
	  for (i=0; i<N; i++){  
	  for (j=0; j<M; j++){ 
			for (it=0; it< TimStep; it++)
		     trace[it]=wave[i+it*N][j];
	   	     FindPeaks(deltat, TimLen, dt_write, trace, LocSta ,LocEnd,LocEnd, v_th, &PeakTim[i*M+j][0], LenPeakTime); //consider only the trace before the LocEnd
	   } 
	   }
	...
	//Visulize the PeakMean	   
     ...
     printf("tau %lf, sigma %lf, FirSamMean %lf, PeakMean %lf category %d\n", tau, sigmas[isigma],  FirSamMean, PeakMean, PhaseDiagram[itau][isigma]);
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
     ...
     printf("tau %lf, sigma %lf, FirSamMean %lf, PeakMean %lf category %d period %lf\n", tau, sigma, FirSamMean, PeakMean, PhaseDiagram[itau][isigma], period[itau][isigma] );
