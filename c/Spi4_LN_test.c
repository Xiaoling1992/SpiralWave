...
fptPhase=fopen("Spi4_LN_Phase.dat","w");
...
for(ic=0;ic<1;ic++){
	...
	tau=log( means[imean]/ sqrt( 1+ variances[ivariance]/ pow(means[imean],2) ) ); 
	sigma=sqrt( log( 1+ variances[ivariance]/ pow(means[imean],2) ) );
    
     
     strcpy(filename, "Spi4_LN"); 
	...
	//****
	printf("breaking point 0, tau %lf, sigma %lf\n", tau, sigma);
	...
	CluRadLin(aNoise,   N, M, 's', -999, -999, -999, aglobal, -999);   //int CluRadLin(double *nvth, int height, int width, char mode, double rho, double phi, double DLow, double median, double lambda);
     printf("breaking porint 0.1, a[0][0]=%lf, a[49][49]=%lf \n", aNoise[0], aNoise[49+49*M]);
    CluRadLin(tauNoise, N, M, 'N', rho, -999, -999, tau, sigma); //-999 means that input is not necessary in the function.
    puts("breaking point 0.2\n");
    CluRadLin(cNoise,   N, M, 's', -999, -999, -999, cglobal, -999);  
    //*******
    printf("breaking porint 1, t[0][0]=%lf, t[49][49]=%lf is\n", tauNoise[0], tauNoise[49+49*M]);
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
	...
	for(j=0;j<N*M;j++){
	 ...
	 //******	   
	 printf("breaking porint 2, PeakTim[0][0]=%lf, PeakTim[1][9]=%lf", PeakTim[0][0], PeakTim[1][9]);	    
