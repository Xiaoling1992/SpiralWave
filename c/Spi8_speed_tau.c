...
double gamma=0;
double rhos[]={0}, taus[]={6, 7, 8, 9, 10, 12, 13, 15, 17, 20, 22, 25, 29, 33, 38, 43, 49, 56, 64, 73, 83, 94, 108, 123, 140, 159, 182, 207, 236, 269, 307, 350, 399, 455, 519, 592, 675, 769, 877, 1000}, sigmas[]={1};
...
int N=100,M=3, Leng=N*M, i_speed=90, coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.

double v_th=0.6;
 
double deltat=0.01,t=0.00,TimLen=200.00,deltax=1, deltay=1, dt_write=0.5;          //$$ TimLen incresed to 400 min. 
...
FILE *fptSpeed=fopen("Spi8_speed_tau.dat", "w");
...
for(ip=0;ip<NumRep; ip++){         
    ...
    for(i=0;i<N;i++){
		for(j=0; j<M; j++){
			aNoise[i*M+j]=aglobal;
			cNoise[i*M+j]=cglobal;
			tauNoise[i*M+j]=tau;
	    }
	}  
      ...
      dxdt_Squ(&EVnST1[0][0],&k1[0][0],aNoise,tauNoise,cNoise,gamma,coefx,coefy,N,M,Leng,deltax,deltay);  
       ...
       dxdt_Squ(&EVnST1[0][0],&k3[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay); 
	    ...
	    if(WheMultiple(t, dt_write, deltat/10)|| equal(t,2*deltat,deltat/10) ){
			for (j=0;j<M;j++){
				if(EVnST[0][i_speed*M+j]>= v_th ){
					speed=i_speed*deltax/t;
					fprintf(fptSpeed,"%lf\n", speed);
					stop=1;
					break;
					}
