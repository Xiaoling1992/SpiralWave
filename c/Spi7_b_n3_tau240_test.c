//: _test: test the function to measure the averageg cluster size:1. all loop should repeat for 2 times; 2. use a small lattice.
...
double gamma=0;
double rhos[]={0}, taus[]={240}, sigmas[]={0.5, 0.6};
...
int N=6,M=6, Leng=N*M, WinHei=35, print=1,coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.
...
double deltat=0.01,t=0.00,TimLen=0,deltax=1, deltay=1, dt_write=2;          //$$ TimLen incresed to 400 min. 
...
double period[Len_sigmas][NumRep], AncSize[Len_sigmas][NumRep], AncSize2[Len_sigmas][NumRep];
fptPhase=fopen("Spi7_n3_Phase_t240_test.dat", "w");
fptPeriod=fopen("Spi7_n3_Period_t240_test.dat", "w");
fptAnc=    fopen("Spi7_n3_Anchor_t240_test.dat", "w");
...
for(itau=0;itau<Len_taus;itau++){
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &AncSize2[0][0]);	
  ...
  if ( LocEnd== -1 )
	...
	for (i=0; i<N; i++){  
	 ...
	 // printf("mean period %lf, peaks %d\n", period[isigma][ip], NumCMP);	  
	   ...
	   printf("rho %lf tau=%lf phi %lf, sigma=%lf ip=%d\n",rho, tau, phi, sigma, ip);  
	  for(i=0; i<N; i++){
		  for(j=0; j<M; j++)
		    printf("%.0lf ", tauNoise[i*M+j]); 
		  puts("\n");   }
	 
	 //Visulize the structrural anchor size.
	 ...
	 for (i=0; i<N; i++){
	  ...
	  AncSize[isigma][ip]=(double) numerator/ (double)denominator;	 
      AncSize2[isigma][ip]=(double) numerator2/ (double)numerator;   
      NumSpi++;
      
      printf("%lf, %lf\n", AncSize[isigma][ip], AncSize2[isigma][ip]);
...
for(i=0; i<Len_sigmas; i++){ 
 for(j=0;j<NumRep;j++)	
	fprintf(fptAnc,"%lf  ", AncSize2[i][j]);
	fprintf(fptAnc, "\n");	
		         }
