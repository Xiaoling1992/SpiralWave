//Spi10* Review_5: Andrew's suggestion: dynamic FirSamMean_th= phi.
//Spi10* review: Death, break-up spiral(be active indefinitly), secondry spiral and normal
...
double gamma=0;
double rhos[]={0}, taus[]={5+EPSILON, 6, 7, 8, 9, 10, 12, 14, 16, 19, 22, 26, 30, 36, 42, 49, 57, 67, 79, 92, 108, 127, 149, 174, 204, 
	240, 281, 329, 386, 452, 530, 621, 728, 853, 1000}, phis[]={0.13, 0.16, 0.19, 0.22, 0.25, 0.28, 0.31, 0.34,
	0.37, 0.40, 0.43, 0.46, 0.49, 0.52, 0.55, 0.58, 0.61, 0.64, 0.67, 0.70, 0.73, 0.76, 0.79, 0.82, 0.85, 0.88, 0.91,
	0.94, 0.97, 1.00};

int  NumRep=20, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]);
...
fptPhase=fopen("Spi10_Phase_TauPhi_Rho0_5.dat", "w");

for(irho=0; irho<Len_rhos; irho++){
for(itau=0;itau<Len_taus;itau++){
  int PhaseDiagram[Len_phis][NumRep];
  IniMat_I(Len_phis, NumRep, -1, &PhaseDiagram[0][0]);
...
for(ic=0;ic<Len_cs;ic++){
	...
	phi=phis[iphi];
     ...
     strcat(filename,"Phi");   sprintf(num, "%d",(int)(phi*1000+EPSILON) );strcat(filename,num);
     ...
     strcat(filename,"_wave_5.dat");	
    strcat(filename2,"_map_5.dat");
...
for(ip=0;ip<NumRep; ip++){         
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  ...
  double FirSamMean=0, FirSamMean_th= phi;  //use dynamic FirSamMean_th= phi
  LocEnd=WheDied_Tim(&wave[0][0], N, M, TimStep, v_th);  
     
  if ( LocEnd== -1 )
       PhaseDiagram[iphi][ip]=0;     //died
  else{
     ...
     FirSamMean/=NumSample;
      ...
      if( FirSamMean< FirSamMean_th){
		 if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
		   PhaseDiagram[iphi][ip]=2;//Spiral-brek up
		 else
		   PhaseDiagram[iphi][ip]=3;
      }
	
     else if( WheNeverDied(&wave[0][0], N, M, TimStep, v_th) )
        PhaseDiagram[iphi][ip]=8;  //secondary-spiral
	 else
	    PhaseDiagram[iphi][ip]=5;     //normal
 ...
 PriMat_I(&PhaseDiagram[0][0], Len_phis, NumRep, Len_phis, NumRep, fptPhase); 	       		      
} //end of loop itau
