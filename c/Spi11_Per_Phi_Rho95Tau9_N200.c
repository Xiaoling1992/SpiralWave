...
double gamma=0;
double rhos[]={0.95}, taus[]={9}, phis[]={0.42,  0.44,  0.46,  0.48,  0.5 ,  0.52,  0.54,  0.56,  0.58, 0.6 ,  0.62,  0.64,  0.66,  0.68,  0.7 ,  0.72,  0.74,  0.76, 0.78,  0.8, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98};

int  NumRep=1000, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]);
...
fptPeriod=fopen("Spi11_Period_Phi_Rho95Tau9_N200.dat", "w");
fptPhi=    fopen("Spi11_PhiAct_Phi_Rho95Tau9_N200.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	phi=phis[iphi];    
     ...
     strcat(filename,"_wave_N200.dat");	
    strcat(filename2,"_map_N200.dat");
      ...
      dxdt_Squ_o4(&EVnST1[0][0],&k1[0][0],aNoise,tauNoise,cNoise,gamma,coefx,coefy,N,M,Leng,deltax,deltay);  
       ...
       dxdt_Squ_o4(&EVnST1[0][0],&k3[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay); 
	    ...
	    if(WheMultiple(t, dt_write, deltat/10)|| equal(t,2*deltat,deltat/10) ){
			...
			if (t> 48-EPSILON2 && (WheSpiral=WheLive_t(&wave[0][0], t, dt_write, N, M, TimStep, v_th) ) ==0 ) //24 is the time when the wave reached the edge.
			    break;			   
		}
