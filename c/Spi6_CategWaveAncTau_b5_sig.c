//Spi6*b5: Convex Hull: typo is verified. consider wave from LocSta->TimLen Convex Hull: if the cells with most peaks are less than 10, then return failure. Anchor size is 0. 
...
fptPhase=fopen("Spi6_b5_Phase_AncTau_delete.dat","w");
fptPeriod=fopen("Spi6_b5_Period_AncTau_delete.dat", "w");
fptAnc=fopen("Spi6_b5_Anchor_AncTau_delete.dat", "w");
...
for(ic=0;ic<1;ic++){
	...
	fptMap=fopen("Spi7_n6_2Rho0t240p500c10_map.dat","r");
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
    ...
    fptWave=fopen("Spi7_n6_2Rho0t240p500c10_wave.dat","r");
	rewind(fptWave);   
