...
double gamma=0;
double means[]={6, 10, 17, 28, 46, 77, 129, 216, 360, 600, 1000, 1669, 2783, 4643, 7744, 12918, 21547, 35941, 59951, 100000},
   stds[]={2, 4, 6, 11, 20, 34, 61, 108, 190, 336, 595, 1051, 1857, 3282, 5800, 10250, 18116, 32016, 56583, 100000};

int  Len_means=20, Len_stds=20, Len_rhos=4, NumRep=1;
...
int PhaseDiagram[Len_means][Len_stds];
fptPhase=fopen("Spi41_b_Phase.dat","w");
...
for(irho=0; irho<Len_rhos; irho++){
  ...
  Ini_Matri_I(Len_means, Len_stds, -1, &PhaseDiagram[0][0]);
...
for(ic=0;ic<1;ic++){
	...
	phi=pow( means[imean]- tau_off, 2 )/( stds[istd]*stds[istd] +pow( means[imean]- tau_off, 2 ) );
    tau=means[imean]+ stds[istd]*stds[istd]/(means[imean]- tau_off); 
     
	
    
     
     strcpy(filename, "Spi41_b"); 
     ...
     strcat(filename,"d");   sprintf(num, "%d",(int)(stds[istd]+EPSILON) );strcat(filename,num);
    ...
    CluRadLin(tauNoise, N, M, 'b', rho, phi, tau_off, tau, sigma); //-999 means that input is not necessary in the function.
