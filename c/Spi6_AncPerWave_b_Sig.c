//Input: Spi47_bRho0t44p38c10_wave.dat
...
double gamma=0;
double rhos[]={0}, taus[]={44}, sigmas[]={0.38};

int NumRep=1, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]);
...
int N=100,M=100, Leng=N*M, WinHei=35, print=1,coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.
...
double deltat=0.01,t=0.00,TimLen=200.00,deltax=1, deltay=1, dt_write=2;          //$$ TimLen incresed to 400 min. 
...
double period[Len_sigmas][NumRep], AncSize[Len_sigmas][NumRep];
fptPhase=fopen("Spi6test_b_Phase.dat", "w");
fptPeriod=fopen("Spi6test_b_Period.dat", "w");
fptAnc=fopen("Spi6test_b_Anchor", "w");
...
for(itau=0;itau<Len_taus;itau++){
  ...
  IniMat_D(Len_sigmas, NumRep, -1, &AncSize[0][0]);	
...
for(ip=0;ip<NumRep; ip++){         
    ...
    // puts("breaking point 1");        
     ...
     printf("1. categotory %d\n", PhaseDiagram[isigma][ip]);
    
    if(PhaseDiagram[isigma][ip]==2 || PhaseDiagram[isigma][ip]==8){
	  int  NoMaxPea, MaxPea=0, peaks, peaks_k, count, LocSta=(int)(0/dt_write+EPSILON),NumCMP=20, CelMaxPeak[2][NumCMP];
	  ...
	  for (peaks=MaxPea; peaks>=1 && count<NumCMP; peaks--){
		  for(k=0; k<N*M && count< NumCMP; k++){
			  if (PeakTim[k][LenPeakTime-1]== peaks){
	  ...
	  printf("Before calculating period i,j, period");   
	  for(k=0; k<NumCMP; k++)
	  ...
	  printf("After calculating period i,j, period");   
	  for(k=0; k<NumCMP; k++)
	  printf("%d, %d, %lf\n", CelMaxPeak[0][k], CelMaxPeak[1][k], PerCMP[k]);
	  
	  
	  int NumPoiIn, ABLR[4], ConHulSize=0;
	  ...
	  printf("%d, %d\n", (int)(ConHull[0][k]+EPSILON2), (int)(ConHull[1][k]+EPSILON2));
	  ...
	  AncSize[isigma][ip]=ConHulSize;	
	  printf("anchor size %lf\n", AncSize[isigma][ip]);  
	  

  }  //End of if it is a spiral wave.
