...
fptPhase=fopen("Spi6_b2_PhaseAncTau.dat","w");
fptPeriod=fopen("Spi6_b2_PeriodAncTau.dat", "w");
fptAnc=fopen("Spi6_b2_AnchorAncTau.dat", "w");
...
for(ic=0;ic<1;ic++){
  ...
  //Categorize the wave
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	 int  NoMaxPea=0, MaxPea=0,  peaks_k, RanNum_k, count=0, LocSta=(int)(50/dt_write+EPSILON),NumCMP=0, CelMaxPeak[2][N*M];
	  double PerCMP[N*M];
	  ...
	  printf("2. MaxPeaks %d\n", MaxPea);
	   //find out all cells with the most peaks
	  for(k=0; k<N*M; k++){
	  ...
	  for(i= ABLR[0]; i<= ABLR[1]; i++){
		  for(j= ABLR[2]; j<= ABLR[3]; j++){  //Area of the anchor: All the off cells inside the anchor.
			ConHulSize+= WheInConHull(&ConHull[0][0], NumPoiIn, NumCMP, (double)i, (double)j);//int WheInConHull(double *ConHull, int NumPoints, int LenConHull, double x, double y);  }
		  }
	  }
