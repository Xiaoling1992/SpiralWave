//Input: Spi47_bRho0t44p38c10_wave.dat, it is in /scratch/rice
...
fptPhase=fopen("Spi6.3_b_Phase_test.dat", "w");
fptPeriod=fopen("Spi6.3_b_Period_test.dat", "w");
fptAnc=fopen("Spi6.3_b_Anchor_test.dat", "w");
...
for(ip=0;ip<NumRep; ip++){         
  ...
  if ( LocEnd== -1 )
	...
	printf("tau %lf,  phi %lf,  FirSamMean %lf, PeakMean %lf category %d\n", tau, phi,  FirSamMean, PeakMean, PhaseDiagram[itau][isigma]);
