//Spi7_b_Eps_Rho0_tau44: prove that we can't find out spiral with noise on epsilon.
...
double aglobal, aLow;  ////phi: percentage of responsive cells; rho, clustering rate, aLow, the parameter of unresponsive cell.                                                 
double tau, tau_off, phi, sigma, rho, rhos[]={0};
double cglobal, cLow;
double gamma=0;
double taus[]={0.003, 0.004, 0.005, 0.006, 0.008, 0.011, 0.014, 0.018, 0.023, 0.030, 0.039, 0.050, 0.065, 
	0.083, 0.108, 0.139, 0.180, 0.232, 0.300, 0.387, 0.500, 0.646, 0.834, 1.077, 1.391, 1.797, 2.321, 2.998, 3.871, 5.000}, 
	sigmas[]={0.13, 0.16, 0.19, 0.22, 0.25, 0.28, 0.31, 0.34, 0.37, 0.40, 0.43, 0.46, 0.49, 0.52, 0.55, 0.58, 0.61, 0.64, 
	0.67, 0.70, 0.73, 0.76, 0.79, 0.82, 0.85, 0.88, 0.91, 0.94, 0.97, 1.00};

int  NumRep=10, Len_rhos=sizeof(rhos)/sizeof(rhos[0]),
     Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]);
...
int N=100,M=100, Leng=N*M, print=1,coefx=1,coefy=1;   //N, the number of rows, M, the number of columns.
...
fptPhase=fopen("Spi7_b_Phase_Rho0_tau44.dat", "w");
...
for(isigma=0;isigma<Len_sigmas;isigma++){ 
	...
	rho=rhos[irho]; 
	cglobal=10; //put variability to epsilon
	cLow=taus[itau];	
	phi=sigmas[isigma];
	
	tau=44; 
	tau_off=tau;	//Tau_off is not applied in LogNormal distribution
	
    sigma=-999;
    
     
     strcpy(filename, "Spi7_b"); 
     strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100+EPSILON) );strcat(filename,num); 
     strcat(filename,"p");   sprintf(num, "%d",(int)(phi*100+EPSILON) );strcat(filename,num);
     strcat(filename,"c");   sprintf(num, "%d",(int)(cglobal) );strcat(filename,num);
     strcat(filename,"cf");   sprintf(num, "%d",(int)(cLow*1000) );strcat(filename,num);
...
for(ip=0;ip<NumRep; ip++){         
    ...
    CluRadLin(tauNoise, N, M, 's', -999, -999, -999, tau, -999); //-999 means that input is not necessary in the function.
    CluRadLin(cNoise,   N, M, 'b', rho, phi, cLow, cglobal, -999);    
