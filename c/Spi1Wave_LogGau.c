//_LogGau: given the mean and variance of Lognormal distribution as the input
...
double tau, taus[]={700, 500, 300, 200, 100, 75, 50, 25, 20, 15, 10}, sigmas[]={};
double cglobal, cs[]={10};
double gamma=0, phi_real;
double phi,rho, phis[]={1};

int  Len_as=1, Len_taus=11, Len_phis=1, NumRep=1;
...
double lambda_a,lambda_b, lambda_c, sigma=log(10);   //phi: percentage of responsive cells; psi, clustering probability, aLow, the parameter of unresponsive cell.
...
for(ic=0;ic<1;ic++){
    ...
    //strcat(filename,"Rho");   sprintf(num, "%d",(int)(rho*100));strcat(filename,num); 
      ...
      strcat(filename,"t");   sprintf(num, "%d",(int)(tau));strcat(filename,num);
         strcat(filename,"s");  sprintf( num, "%d",(int)(exp(sigma) ) );strcat(filename,num);
...
{    
	   	     for (i=0;i<N*M;i++){		                    
		     aNoise[i]=aglobal; 
		     tauNoise[i]=exp( gaussrand_K()*sigma+ log(tau) );
