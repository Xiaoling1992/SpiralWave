...
fptPhase=fopen("Spi6_b5_Phase_AncTau.dat","w");
fptPeriod=fopen("Spi6_b5_Period_AncTau.dat", "w");
fptAnc=fopen("Spi6_b5_Anchor_AncTau.dat", "w");
 ...
 //Categorize the wave     
  ...
  if ( LocEnd== -1 )
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  int  NoMaxPea=0, MaxPea=0,  peaks_k, count=0, LocSta=(int)(0/dt_write+EPSILON),NumCMP=0, CelMaxPeak[2][N*M];
	  double PerCMP[N*M];
	...
	for (i=0; i<N; i++){  
	  ...
	  if(NumCMP<10)  //If the cells with most peaks are too few, instead of pick up these cell with less peaks, we will return failure.
		 ...
		 // puts("\n");
		  ...
		  printf("conve hull: left %d, right %d top %d bottom %d cells %d, anchor size %lf\n", ABLR[2],ABLR[3],ABLR[0],ABLR[1], NumPoiIn, AncSize[itau][isigma]);   
         } 
