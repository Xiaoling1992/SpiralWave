...
double gamma=0;
double rhos[]={0}, taus[]={44}, sigmas[]={0.28,0.30,0.32,0.34,0.36,0.38,0.40,0.42,0.44,0.46,0.48,0.50,0.52,0.54,0.56,0.58,0.60,0.62};

int  NumRep=250, Len_as=sizeof(as)/sizeof(as[0]), Len_cs=sizeof(cs)/sizeof(cs[0]),  Len_rhos=sizeof(rhos)/sizeof(rhos[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_sigmas=sizeof(sigmas)/sizeof(sigmas[0]);
...
fptPhase=fopen("Spi61_b2_Phase.dat", "w");
fptPeriod=fopen("Spi61_b2_Period.dat", "w");
fptAnc=fopen("Spi61_b2_Anchor.dat", "w");
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
    ...
    if(PhaseDiagram[isigma][ip]==2 || PhaseDiagram[isigma][ip]==8){
	  int  NoMaxPea=0, MaxPea=0, peaks, peaks_k, count=0, LocSta=(int)(100/dt_write+EPSILON),NumCMP=20, CelMaxPeak[2][NumCMP];
	  ...
	  for(i= ABLR[0]; i<= ABLR[1]; i++){
		  for(j= ABLR[2]; j<= ABLR[3]; j++){  //Area of the anchor: All the off cells inside the anchor.
			ConHulSize+= (tauNoise[i*M+j]==tau_off)*WheInConHull(&ConHull[0][0], NumPoiIn, NumCMP, (double)i, (double)j)*SearchClu_G4_Squ(tauNoise, state, i, j, N, M, N, M, tau_off, cluster);//int WheInConHull(double *ConHull, int NumPoints, int LenConHull, double x, double y);  }
		  }
	  }
