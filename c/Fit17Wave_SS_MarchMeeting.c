//March meeting 2018, los angeles: high excitability and low excitability
...
double aglobal,as[]={0.01, 0.1};  //WT, trkA, sinR, ktrA
double tau, taus[]={300};
double cglobal, cs[]={10};
double gamma=0, phi_real;
double phi,rho, phis[]={0.5};

int  Len_as=sizeof(as)/sizeof(as[0]), Len_taus=sizeof(taus)/sizeof(taus[0]), Len_phis=sizeof(phis)/sizeof(phis[0]), NumRep=1;
...
double deltat=0.01,t=0.00,TimLen=200.00,deltax=1, deltay=2, dt_write=2;          //$$ TimLen incresed to 400 min.    
