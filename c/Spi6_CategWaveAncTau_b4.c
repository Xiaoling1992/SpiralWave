...
fptPhase=fopen("Spi6_b4_Phase_AncTau.dat","w");
fptPeriod=fopen("Spi6_b4_Period_AncTau.dat", "w");
fptAnc=fopen("Spi6_b4_Anchor_AncTau.dat", "w");
 ...
 //Categorize the wave     
  ...
  if ( LocEnd== -1 )
    ...
    if(PhaseDiagram[itau][isigma]==2 || PhaseDiagram[itau][isigma]==8){
	  int  NoMaxPea=0, MaxPea=0,  peaks_k, count=0, LocSta=(int)(10/dt_write+EPSILON),NumCMP=0, CelMaxPeak[2][N*M];
	  double PerCMP[N*M];
