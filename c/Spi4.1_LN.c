...
fptPhase=fopen("Spi41_LN_Phase.dat","w");
...
for(ic=0;ic<1;ic++){
	...
	tau=log( means[imean]/ sqrt( 1+ stds[istd]*stds[istd]/ means[imean]/means[imean] ) ); 
	sigma=sqrt( log( 1+ stds[istd]*stds[istd]/ means[imean]/means[imean] ) );
    
     
     strcpy(filename, "Spi41_LN"); 
