...
double gamma=0;
double rhos[]={0.75}, taus[]={7, 8, 9, 10, 12, 14, 16, 19, 22, 26, 30, 36, 42, 49, 57, 67, 79, 92, 108, 127, 149, 174, 204, 
...
fptPhase=fopen("Spi10_Phase_TauPhi_Rho75.dat", "w");
...
for(ip=0;ip<NumRep; ip++){         
 ...
 //$$ 4. Change the Trigger Time,length, and number of rows $$$$$$$$$$$$$$$$*//  
  while (t<TimLen) { 
    ...
    // Categorize the wave     
  int LocEnd,  NumSample=N*M, LenPeakTime=30, NumSpiral=0, it;
  ...
  if ( LocEnd== -1 )
   ...
   #pragma omp parallel private(j, it)
   {
	 ...
	 for(i=0; i<NumSample; i++)
	     NumSpiral+= (PeakTim[i][LenPeakTime-1] >1+ EPSILON2);
	 if(NumSpiral >= 1)
