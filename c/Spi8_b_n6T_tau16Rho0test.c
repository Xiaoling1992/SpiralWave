//Spi7: Parallel the process of categorizing and period. 
...
double gamma=0;
double rhos[]={0, 0.75}, taus[]={16}, sigmas[]={0.42, 0.6};
...
fptPhase=fopen("Spi8_Phase_t16Rho0test.dat", "w");
fptPeriod=fopen("Spi8_Period_t16Rho0test.dat", "w");
fptAnc=    fopen("Spi8_Anchor_t16Rho0test.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	sigma=-999;
     ...
     strcat(filename,"_wave_test.dat");	
    strcat(filename2,"_map_test.dat");
...
printf("Finishing generating the biofilm, costs %ld seconds of time\n",time(NULL)-tt);
 ...
 printf("Finishing generating wave, costs %ld seconds of time\n",time(NULL)-tt);
...
printf("Finishing generating the fraction of firing cells %lf, Locend %ld, costs %ld seconds of time\n",FirSamMean, LocEnd, time(NULL)-tt);
  ...
  #pragma omp parallel private(j, it)
  {
	...
	PeakMean/=NumSample;
	 printf("Finishing generating the PeakMean %lf, costs %ld seconds of time\n",PeakMean, time(NULL)-tt);  
...
printf("Finishing generating the phase, costs %ld seconds of time\n",time(NULL)-tt);
    ...
    int state[N*M], cluster[N*M], numerator=0, denominator=0, numerator2=0,CluSiz;
     ...
     printf("Finishing generating the structural anchor, costs %ld seconds of time\n",time(NULL)-tt);
  ...
  printf("Finishing generating the PeakTim, costs %ld seconds of time\n",time(NULL)-tt);
	  ...
	  printf("Finishing generating the period, costs %ld seconds of time\n",time(NULL)-tt);
	   //for(k=0; k<NumCMP; k++)
