...
srand((unsigned) time(&tt));  //Initiating random number generator only once. 
	...
	fptMap=fopen("Spi7_n5Rho0t240p320c10_map.dat","r"); //It must be "r"
	fptOCC=fopen("Spi7_n5Rho0t240p320c10_OCCin.dat", "w");
    for(i=0; i<N; i++){
			for(j=0; j<M; j++){
    ...
    int state[N*M], cluster[N*M], CluSiz;
	  	  ...
	  	  int NumPoiIn, NumCMP=8;  //Accumulative parameter
		  double ConHull[2][8]={ {76, 66, 64, 60, 52, 64, 67, 70},{54, 38, 26, 35, 46, 59, 67, 61} };
		  NumPoiIn=NumCMP;
		 ...
		 for (i=0; i<N; i++){
		  ...
		  printf("Test WheInConHull, %d\n", WheInConHull( &ConHull[0][0], NumPoiIn, NumCMP, (double)(65), (double)(50) ) );
