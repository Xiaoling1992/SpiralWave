...
double tau, tau_off, phi, rho, mean, std;
...
fptPhase=fopen("Spi10_Phase_MeaVar_b.dat", "w");
...
for(ic=0;ic<Len_cs;ic++){
	...
	phi=pow( mean- tau_off, 2 )/( std*std +pow( mean- tau_off, 2 ) );
    tau=mean+ std*std/(mean- tau_off); 
     ...
     strcat(filename,"_wave_b.dat");	
    strcat(filename2,"_map_b.dat");
