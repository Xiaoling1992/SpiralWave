//4.6: Trigger one column of cell with v=1 and 3 columns behind it with w=1 to create the spiral wave.
//4.5: 1. Create an unidirectional diode. 2. Increase the diameters of anchor. 
//4.4: Use different boundary condition systems. Reflective for 3 sides except the bottom (obserbing). set a non-penetrating wall (tau~0) above of the anchor to divide the top half into two to delay the propagation of wave to the right side 
//4.3: Test the relationship between period of spral wave and the diameter. Trigger the central area with radius r, and trigger half of the first 2 rows.
...
double tau, tau_off, phi, sigma, rho, rhos[]={0, 0.25, 0.5, 0.75};
...
double anchor, anchors[]={4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80}; //This is the diameter of the anchor. 

int  Len_taus=30, Len_sigmas=30, Len_rhos=4, NumRep=1, Len_anchors=20;
int i,j, ia, ic, itau, isigma, irho, ip, ianchor; //i, i is only used for any short, self-closed loop. 
...
for(ic=0;ic<1;ic++){
    ...
    for (i=0; i<N; i++){
			for(j=0; j<M; j++){
				if( pow(i-(N-1.0)/2.0, 2)+pow(j-(M-1.0)/2.0, 2)<=anchor*anchor/4)
		         tauNoise[i*M+j]=tau_off; 
		       // else if(i<= (N-1)/2- anchor/2-1 && (M-1)/2-j >=0 && (M-1)/2-j <= anchor/2 )
		        // tauNoise[i*M+j]=1;
		         else
		         tauNoise[i*M+j]=tau; 
		   }	   
  ...
  while (t<TimLen) { 
	...
	//MaxColumn=LargestCluster(tauNoise, N, M, N,M, 0, tau_off, tau, 5);
     ...
     if(fabs(t-0.00)<EPSILON2){     //Because t is a variable, it might have some little error.   
		 for (i=0; i<N; i++){
			for(j=0; j<M; j++){
				if( pow(i-(N-1.0)/2.0, 2)+pow(j-(M-1.0)/2.0, 2) > anchor*anchor/4 && i< (N-1.0)/2 && j==M/2)
		         EVnST[0][i*M+j]=1; 
		        else if( pow(i-(N-1.0)/2.0, 2)+pow(j-(M-1.0)/2.0, 2) > anchor*anchor/4 && i< (N-1.0)/2 && j==M/2+1 )
		         EVnST[1][i*M+j]=0.2; 
		     }	   
         }
       		    }     // Trigger: pulse v:1;        
	
    dxdt_Squ(&EVnST[0][0],&k0[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay); 
	...
	for(j=0;j<N*M;j++){
      ...
      dxdt_Squ(&EVnST1[0][0],&k1[0][0],aNoise,tauNoise,cNoise,gamma,coefx,coefy,N,M,Leng,deltax,deltay);  
		#pragma omp parallel for private(i)
		for(j=0;j<N*M;j++){
			for(i=0;i<2;i++)
       ...
       dxdt_Squ(&EVnST1[0][0],&k2[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay);
        #pragma omp parallel for private(i)
        for(j=0;j<N*M;j++)  {
			for(i=0;i<2;i++)
            EVnST1[i][j]=EVnST[i][j]+2*deltat*k2[i][j];  }   //E1,V1... calculated by k0;       

       dxdt_Squ(&EVnST1[0][0],&k3[0][0],aNoise,tauNoise,cNoise,gamma, coefx,coefy,N,M,Leng, deltax,deltay); 
        #pragma omp parallel for private(i)
        for(j=0;j<N*M;j++)  {
			for(i=0;i<2;i++)
    ...
    int k, it, NumSample=(int)(2.8*N), LenPeakTime=10;
     double theta, radius=9.0/19.0*N, trace[TimStep], PeakTim[NumSample][LenPeakTime];     
    ...
    // puts("breaking point 1");        
     ...
     printf("anchor %lf, tau %lf, FirSamMean %lf, PeakMean %lf category %d\n", anchor, tau, FirSamMean,PeakMean, PhaseDiagram[itau][isigma]);
