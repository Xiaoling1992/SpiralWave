...
double gamma=0;
double taus[]={1000}, sigmas[]={0.22,0.24,0.26,0.28,0.30,0.32,0.34,0.36,0.38,0.40,0.42,0.44,0.46,0.48,0.50,0.52,0.54,0.56,0.58,0.60,0.62,0.64,0.66,0.68,0.70,0.72,0.74,0.76,0.78,0.80,0.82,0.84,0.86,0.88,0.90,0.92,0.94,0.96,0.98,1.00};

int  Len_taus=1, Len_sigmas=40, Len_rhos=1, NumRep=100;
...
int PhaseDiagram[Len_taus][Len_sigmas], period[Len_taus][Len_sigmas];
fptPhase=fopen("Spi47_b_Phase_t1000.dat", "w");
fptPeriod=fopen("Spi47_b_Period_t1000.dat", "w");
...
for(irho=0; irho<Len_rhos; irho++){
  ...
  Ini_Matri_I(Len_taus, Len_sigmas, -1, &period[0][0]);
...
//for(itau=0;itau<Len_taus;itau++){ 	
 ...
 for(i=0; i<Len_taus; i++){ 
 for(j=0;j<Len_sigmas;j++)	
	fprintf(fptPeriod,"%d  ", period[i][j]);
	fprintf(fptPeriod, "\n");	
		         }
