//5.22: tau=240; p_0=0.25
//Spi5.32: p_0=1-0.593=0.27
//Spi5.31: p_0=1-0.593=0.407; Only print the trace of the first spiral wave; Use sizeof to calculate the lenth of each parameter array.
...
double gamma=0;
double rhos[]={0}, taus[]={240}, sigmas[]={0.255,0.256,0.257,0.259,0.26,0.262,0.265,0.268,0.271,0.275,0.28,0.286,0.293,0.301,0.311,0.323,0.338,0.355,0.375,0.4};
...
FILE *fptPhase, *fptPeriod, *fptWave, *fptMap;     //fpt: ThT; fpt2: parameter;
...
double period[Len_sigmas][NumRep];
fptPhase=fopen("Spi522_b_Phase.dat", "w");
fptPeriod=fopen("Spi522_b_Period.dat", "w");
   
  
for(irho=0; irho<Len_rhos; irho++){
for(itau=0;itau<Len_taus;itau++){
	
  Ini_Matri_I(Len_sigmas, NumRep, -1, &PhaseDiagram[0][0]);
  Ini_Matri(Len_sigmas, NumRep, -1, &period[0][0]);	
  ...
  while (t<TimLen) { 
    ...
    if(PhaseDiagram[isigma][ip]==2 || PhaseDiagram[isigma][ip]==8){
	  ...
	  for (k=0; k<N*M; k++){
	      ...
	      period[isigma][ip]=(PeakTim[NoMaxPea][MaxPea-1]- PeakTim[NoMaxPea][0])/(MaxPea-1);
