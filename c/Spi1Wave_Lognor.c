...
double tau, taus[]={2.2, 2.6, 3.0, 3.4, 3.8, 4.2, 4.6, 5.0, 5.4, 5.8, 6.2, 6.4};  //\mu of log-normal distribution
double sigma, sigmas[]={0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3};
double cglobal, cs[]={10};
double gamma=0, phi_real;
double phi,rho, phis[]={1};

int  Len_as=1, Len_taus=12, Len_sigmas=12, Len_phis=1, NumRep=1;
...
int N=100,M=100,WinHei=35, print=1,noise=4,coefx,coefy;   
...
for(ic=0;ic<1;ic++){
    ...
    //  strcat(filename,"a"); sprintf(num, "%d",(int)(aglobal*1000)); strcat(filename,num); 
      strcat(filename,"mu");   sprintf(num, "%d",(int)(tau*100));strcat(filename,num);
