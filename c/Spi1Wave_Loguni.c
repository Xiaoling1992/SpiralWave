//_Loguni: use \tau and sigmas to generate log uniformly distributed noise. log(tau) is uniformly distributed between log(tau_0)-log(sigma) to log(tau_0)+log(sigma)
...
double tau, taus[]={700, 500, 300, 200, 100, 75, 50, 25, 20, 15, 10};  //\mu of log-normal distribution
double sigma, sigmas[]={2, 4, 8, 12, 16, 20, 24, 28, 32};
double cglobal, cs[]={10};
double gamma=0, phi_real;
double phi,rho, phis[]={1};

int  Len_as=1, Len_taus=11, Len_sigmas=9, Len_phis=1, NumRep=1;
...
int N=100,M=100,WinHei=35, print=1,noise=2,coefx,coefy;   
...
double lambda_a=1,lambda_b=1, lambda_c;   //phi: percentage of responsive cells; psi, clustering probability, aLow, the parameter of unresponsive cell.
...
for(ic=0;ic<1;ic++){
    ...
    //  strcat(filename,"a"); sprintf(num, "%d",(int)(aglobal*1000)); strcat(filename,num); 
      strcat(filename,"tau");   sprintf(num, "%d",(int)(tau+EPSILON));strcat(filename,num);
         strcat(filename,"l");  sprintf( num, "%d",(int)(sigma+EPSILON) );strcat(filename,num);
      strcat(filename,"c");   sprintf(num, "%d",(int)(cglobal*100+EPSILON));strcat(filename,num);
    ...
    do{
     ...
     CluGen_G5_1(cNoise, N, M, phi, rho, cLow, cglobal, 1.5873*rho+1, 1.5873*rho+1, 0, 1, 0);
      ...
      while  ( fabs(phi_real-phi) > 0.003 );
		     ...
		     aNoise[i]=aglobal; 
		     tauNoise[i]=exp(DisUni(log(tau),log(sigma)));
		     cNoise[i]=cglobal;
		     }                     //Because responsive cell has a=0.2 (DHigh), while unresponsive cell has a=0.3 (aLow), DHigh<aLow. Thus make DHigh=-0.2, aLow=-0.3. Finally reverse the values. 	
		}
...
{    
	   	     for (i=0;i<N*M;i++){		                    
		     aNoise[i]=aglobal; 
		     tauNoise[i]=exp( gaussrand_K()*sigma+ tau );
		     cNoise[i]=cglobal; }                     //Because responsive cell has a=0.2 (DHigh), while unresponsive cell has a=0.3 (aLow), DHigh<aLow. Thus make DHigh=-0.2, aLow=-0.3. Finally reverse the values. 	
		}
