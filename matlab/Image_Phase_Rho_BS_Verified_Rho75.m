...
for i=1:length(taus)*length(phis)
    ...
    if( taus(itau) >=26 )
        ...
        phase_Pre(i,1)= sum( data(i,:)==0 );
        phase_Pre(i,2)= sum( data(i,:)==3 )+sum( data(i,:)==5 )+sum( data(i,:)==9 ); %When tau is small, the "secondary" is in fact normal
        phase_Pre(i,3)= sum( data(i,:)==2 );
...
phase(taus==57, phis==0.55) =3;  %This is BS
...
print(gcf,['../PapFig/Image_Phase_TauPhi_Rho75_BS_Ver.png'],'-dpng','-r400');
