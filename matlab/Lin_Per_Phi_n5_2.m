%figure 5A, period as function of phi in percolation theory. Prediction and
%simulation.
clear all;
pi=3.1415926;
epsilon=10;
...
figure;
plot(phis(phis>0.3 ), anchor(1,phis>0.3 )*pi/sqrt(epsilon/2), 'LineStyle','--', 'Color','g', 'marker', 'O','linewidth',3,'markersize' ,4, 'MarkerFacecolor','g');
hold on
errorbar(phis( phis>0.3 ), period(1,phis>0.3 ), period(2, phis>0.3), 'LineStyle','none', 'Color','g', 'marker', 'O', 'linewidth',3,'markersize' ,7,'MarkerFacecolor','g');
legend(gca, {'\ prediction','\ simulation'}, 'FontSize',15, 'FontWeight', 'bold','interpreter', 'latex','location', 'southwest');
legend boxoff
plot([0.41, 0.41], [0, 120], 'LineStyle','--', 'LineWidth', 2, 'Color', 'k');
...
text(phis(phis==0.32)-0.005, period(1, phis==0.32)+8, 'B','FontSize',15, 'FontWeight', 'bold')
text(phis(phis==0.42)-0.005, period(1, phis==0.42)+8, 'C','FontSize',15, 'FontWeight', 'bold')
text(phis(phis==0.54)-0.005, period(1, phis==0.54)+8, 'D','FontSize',15, 'FontWeight', 'bold')
...
print(gcf,['../PapFig/Lin_PerStruAnc_Phi_2.png'],'-dpng','-r400');
